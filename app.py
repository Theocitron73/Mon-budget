...
def reinitialiser_modifications():
    # On vide le témoin
    if "df_temoin" in st.session_state:
        st.session_state.df_temoin = {}
    
    # On supprime toutes les clés liées aux widgets (catégories et mois)
    for key in list(st.session_state.keys()):
        if key.startswith("cat_") or key.startswith("mo_"):
...
def update_mots_cles_neon(categorie, liste_mots):
    try:
        with engine.connect() as conn:
            # Cette requête marche pour les catégories par défaut ET personnalisées
            conn.execute(
                text("""
...
def charger_dictionnaire_mots_cles():
    """Récupère et nettoie le dictionnaire depuis Neon."""
    try:
        with engine.connect() as conn:
            res = conn.execute(text("SELECT categorie, mots_cles FROM config_categories")).fetchall()
            ...
            for r in res:
                ...
                # Si PostgreSQL renvoie une liste Python (cas idéal)
                if isinstance(mots, list):
                    dictionnaire_propre[cat] = mots
                # Si PostgreSQL renvoie une string "{mots,clés}" (ton cas actuel)
                elif isinstance(mots, str):
                    clean_mots = mots.replace("{", "").replace("}", "").replace('"', '')
                    dictionnaire_propre[cat] = [m.strip() for m in clean_mots.split(",")]
                else:
                    dictionnaire_propre[cat] = []
                    
            return dictionnaire_propre
    except Exception as e:
...
def categoriser(nom_operation, montant=0, compte_actuel=None, ligne_complete=None):
    ...
    dictionnaire_auto = charger_dictionnaire_mots_cles()
    
    # Scan des catégories et des mots-clés
    for cat, mots in dictionnaire_auto.items():
...
def sauvegarder_donnees_neon(nouveau_df, user=None, dict_noms_originaux=None):
    try:
        u_final = user or st.session_state.get("user")
        if not u_final: return False

        df_to_save = nouveau_df.copy()
        u_final_str = str(u_final).strip()
        ...
        with engine.begin() as conn_sql:
            for idx, row in df_to_save.iterrows():
                ...
                # --- SUPPRESSION SI RENOMMÉ ---
                if dict_noms_originaux and idx in dict_noms_originaux:
                    nom_anc = str(dict_noms_originaux[idx]).strip()
                    if nom_anc != str(row['nom']).strip():
                        # On utilise 'u_id' au lieu de 'utilisateur' dans le paramètre pour tester
                        delete_query = text("""
                            DELETE FROM transactions 
                            WHERE date = :d 
                            AND nom = :n_anc 
                ...
                # --- INSERTION / UPSERT (Paramètres renommés sans accents) ---
                query = text("""
                    INSERT INTO transactions (date, nom, montant, categorie, compte, utilisateur, mois, année)
                    VALUES (:d, :n, :m, :cat, :compte, :u, :mois, :an)
                ...
                # On mappe manuellement chaque valeur à un nom de paramètre simple
                params = {
                    "d": row['date'],
                    "n": str(row['nom']).strip(),
                    "m": float(row['montant']),
                    "cat": str(row.get('categorie', 'Inconnu')),
                    "compte": str(row.get('compte', 'Inconnu')),
                    "u": u_final_str,
                    "mois": str(row.get('mois', 'Inconnu')),
                    "an": int(row['année']) if pd.notnull(row.get('année')) else 0
                }
        ...
        # On affiche l'erreur complète pour débugger si ça persiste
...
def calculer_patrimoine_complet(df_global, config_comptes, annee_cible):
    """
    Calcule l'évolution de tous les comptes depuis le début.
    Retourne : 
    - soldes_au_1er_janvier : dict des soldes au début de l'annee_cible
    - evolution_annee : dict des soldes mois par mois pour l'annee_cible
    """
    # 1. Initialisation avec les soldes de la config (Point Zéro)
    # Remplacer la ligne problématique par celle-ci :
    soldes_courants = {}
    for k, v in config_comptes.items():
        nom_c = str(k).strip().upper()
        # Si v est un dictionnaire (config), on prend .get("Solde")
        if isinstance(v, dict):
            soldes_courants[nom_c] = float(v.get("Solde", 0.0))
        # Si v est déjà un chiffre (numpy.float64 ou int), on le prend direct
        else:
            soldes_courants[nom_c] = float(v)
    
    # 2. On récupère la liste de toutes les années présentes, triées
    toutes_annees = sorted(df_global['année'].unique())
    noms_mois = ["Janvier", "Février", "Mars", "Avril", "Mai", "Juin", 
                 "Juillet", "Août", "Septembre", "Octobre", "Novembre", "Décembre"]
    
    soldes_au_1er_janvier_cible = {}
    evolution_annee_cible = {c: [] for c in soldes_courants.keys()}

    for annee in toutes_annees:
        if annee > annee_cible: break
            
        # Si on arrive au 1er janvier de l'année demandée, on sauvegarde l'état
        if annee == annee_cible:
            soldes_au_1er_janvier_cible = soldes_courants.copy()

        for mois in noms_mois:
            df_mois = df_global[(df_global['année'] == annee) & (df_global['mois'] == mois)]
            
            for _, ligne in df_mois.iterrows():
                montant = float(ligne.get('montant', 0))
                compte_src = str(ligne.get('compte', '')).strip().upper()
                cat = str(ligne.get('categorie', '')).upper()
                nom_trans = str(ligne.get('nom', '')).upper()
                
                # On fusionne tout pour ne rien rater
                texte_integral = f"{nom_trans} {cat}"

                # 1. Impact sur le compte émetteur
                if compte_src in soldes_courants:
                    soldes_courants[compte_src] += montant
                
                # 2. Simulation de la contrepartie (Virements Internes)
                if "🔄" in cat or "VERS" in cat:
                    # A. On identifie le PROFIL (Groupe) de la source
                    nom_src_cfg = next((k for k in config_comptes.keys() if k.strip().upper() == compte_src), None)
                    groupe_cible = config_comptes[nom_src_cfg].get("Groupe") if nom_src_cfg else None

                    if not groupe_cible:
                        print(f"⚠️ Impossible de trouver le groupe pour {compte_src}, virement ignoré.")
                        continue

                    # B. Nettoyage du texte pour la recherche
                    texte_recherche = texte_integral.replace(compte_src, "")
                    
                    

                    meilleur_match = None
                    
                    # C. On ne cherche QUE dans les comptes du MÊME GROPE
                    for nom_dest, cfg_dest in config_comptes.items():
                        dest_upper = str(nom_dest).strip().upper()
                        
                        # Sécurités de base
                        if dest_upper == compte_src: continue
                        if cfg_dest.get("Groupe") != groupe_cible: continue # BLOQUE LE MÉLANGE DES PROFILS

                        # D. Stratégie de match précise
                        # On ignore les mots qui font faire des erreurs
                        mots_a_ignorer = ["CCP", "VERS", "VIREMENT", "EPARGNE", "THEO", "AUDE"]
                        mots_compte = [m for m in dest_upper.split() if len(m) >= 3 and m not in mots_a_ignorer]
                        
                        # Si le nom complet est dans le texte OU si un mot unique match
                        if dest_upper in texte_recherche or (mots_compte and any(m in texte_recherche for m in mots_compte)):
                            meilleur_match = dest_upper
                            break

                    # E. Application du mouvement si trouvé
                    if meilleur_match:
                        # Vérification anti-doublon (pour ne pas compter 2x si déjà dans le réel)
                        deja_present = not df_global[
                            (df_global['année'] == annee) & 
                            (df_global['mois'] == mois) &
                            (df_global['compte'].str.upper() == meilleur_match) & 
                            (abs(df_global['montant'] - (-montant)) < 0.1)
                        ].empty

                        
                        if not deja_present:
                            soldes_courants[meilleur_match] -= montant
                            #print(f"✅ MATCH RÉUSSI ({groupe_cible}) : {compte_src} -> {meilleur_match}")
                        else:
                            print(f"⚠️ DOUBLON : La ligne existe déjà sur {meilleur_match}")
                    else:
                        print(f"❌ DESTINATION NON TROUVÉE dans le groupe {groupe_cible}")
            # Si on est dans l'année cible, on enregistre pour le graphique
            if annee == annee_cible:
                for c in soldes_courants:
                    evolution_annee_cible[c].append(soldes_courants[c])

    return soldes_au_1er_janvier_cible, evolution_annee_cible
   ...
   # 1. On sécurise le DataFrame
    ...
    selected = option_menu(
        ...
        styles={
            "container": {
                ...
                "icon": {"color": "#5092d8", "font-size": "25px"},
                "nav-link": {
                    "font-size": "15px",
    ...
    if selected == "Analyses":
            ...
            def afficher_dashboard():
                ...
                options_profil = ["Tous"] + groupes_reels if len(groupes_reels) > 1 else (groupes_reels if groupes_reels else ["Tous"])

                with cols_filtres[0]:
                    choix_actuel = st.pills("🎯 Profil", options_profil, selection_mode="single", 
                                            default=options_profil[st.session_state.filtre_profil_index], key="choix_g_widget")
                    if not choix_actuel: 
                ...
                # --- 2. DÉTERMINATION DES COMPTES DU PROFIL ---
                if choix_actuel != "Tous":
                    profil_recherche = str(choix_actuel).strip().lower()
                    cps = [c for c, cfg in st.session_state.config_groupes.items() 
                ...
                # --- 3. SÉLECTEUR D'ANNÉE ---
                df_h = st.session_state.df.copy()
                annees_dispo = sorted([int(a) for a in df_h['année'].dropna().unique()], reverse=True) if not df_h.empty else [pd.Timestamp.now().year]
                
                with cols_filtres[1]:
                    annee_choisie = st.pills("📅 Année", options=annees_dispo, default=annees_dispo[0], key="annee_widget")
                    if not annee_choisie: annee_choisie = annees_dispo[0]

                # --- 4. CALCUL UNIQUE (LA SOURCE DE VÉRITÉ) ---
                # On calcule tout l'historique pour l'année choisie
                report_janvier, evolution_mensuelle = calculer_patrimoine_complet(
                    st.session_state.df, 
                    st.session_state.config_groupes, 
                    annee_choisie
                )
                
                # --- 5. PRÉPARATION DU TABLEAU RÉCAPITULATIF (df_tab) ---
                df_tab = pd.DataFrame({'mois': noms_mois_list})
                for c in cps:
                    nom_c_upper = str(c).strip().upper()
                    # Si le compte a une évolution calculée, on la prend, sinon on met le report fixe
                    if nom_c_upper in evolution_mensuelle:
                        df_tab[c] = evolution_mensuelle[nom_c_upper]
                    else:
                        val_init = report_janvier.get(nom_c_upper, 0.0)
                        df_tab[c] = [val_init] * 12

                # Somme du patrimoine pour le profil choisi
                df_tab['Patrimoine'] = df_tab[cps].sum(axis=1)

                # --- 6. RÉCUPÉRATION DES FLUX (REVENUS/DÉPENSES) ---
                df_dash = df_h[df_h["année"] == annee_choisie].copy()
                # Filtrer df_dash pour ne garder que les comptes du profil (cps)
                cps_upper = [c.upper() for c in cps]
                df_dash = df_dash[df_dash['compte'].str.upper().isin(cps_upper)]

                # Identification des virements pour les exclure des revenus/dépenses
                virements_tech = [cat for cat in df_dash['categorie'].unique() if any(x in str(cat).upper() for x in ["🔄", "VERS ", "INTERNE"])]
                df_flux = df_dash[~df_dash["categorie"].isin(virements_tech)]

                if not df_flux.empty:
                    df_ann = df_flux.groupby('mois').agg(
                ...
                # --- CALCUL DU SOLDE À AFFICHER (Mois choisi) ---
                # On cherche l'index du mois sélectionné pour savoir quelle valeur extraire
                ...
                idx_mois = noms_mois_list.index(mois_choisi) if mois_choisi in noms_mois_list else 0

                # --- MODIFICATION ICI ---
                # On utilise [-1] au lieu de [idx_mois] pour avoir la fin d'année/situation actuelle
                solde_global_kpi = df_tab['Patrimoine'].iloc[-1] 

                st.write(f"#### 🏦 Situation Financière {annee_choisie} : {choix_actuel} ")

                cols_kpi = st.columns(len(cps) + 1)

                # 1. CARTE SOLDE GLOBAL (Toujours à jour)
                with cols_kpi[0]:
                    st.markdown(f"""
                        <div style="background-color: {col_patri}; padding: 15px; border-radius: 12px; text-align: center;">
                            <p style="margin: 0; font-size: 12px; color: white; font-weight: bold;">💰 SOLDE GLOBAL</p>
                            <p style="margin: 0; font-size: 20px; color: white; font-weight: 800;">{solde_global_kpi:,.2f} €</p>
                        </div>
                    """, unsafe_allow_html=True)

                # 2. CARTES INDIVIDUELLES (Toujours à jour)
                for i, c in enumerate(cps):
                    # --- MODIFICATION ICI AUSSI ---
                    val_compte = df_tab[c].iloc[-1] 
                    
                    couleur_c = st.session_state.config_groupes.get(c, {}).get("Couleur", "#3498db")
                    
                    with cols_kpi[i+1]:
                        st.markdown(f"""
                            <div style="background-color: {couleur_c}; padding: 15px; border-radius: 12px; text-align: center;">
                                <p style="margin: 0; font-size: 11px; color: white; font-weight: bold;">{c.upper()}</p>
                                <p style="margin: 0; font-size: 18px; color: white; font-weight: 700;">{val_compte:,.2f} €</p>
                            </div>
                        """, unsafe_allow_html=True)
                ...
                with c_recap:
                    ...
                    is_vir = df_m['categorie'].str.upper().isin([c.upper() for c in virements_tech])
                    df_virs = df_m[is_vir]
                    ...
                    with t_graph:
                        # --- RECRÉATION DE LA LISTE DES CATÉGORIES (SÉCURITÉ) ---
                        # On utilise df_dash qui contient déjà les transactions filtrées pour l'année et le profil
                        if not df_dash.empty:
                            # 1. On récupère toutes les catégories uniques
                            toutes_cats = df_dash['categorie'].unique().tolist()
                            
                            # 2. On identifie les catégories de virements (pour les masquer)
                            virements_tech = [
                                c for c in toutes_cats 
                                if any(x in str(c).upper() for x in ["🔄", "VERS ", "INTERNE", "VIREMENT"])
                            ]
                            
                            # 3. La liste finale pour tes filtres
                            categories_dispo = sorted([
                                c for c in toutes_cats 
                                if c not in virements_tech and str(c) != "nan"
                            ])
                        else:
                            categories_dispo = []

                        # --- MAINTENANT TON CODE EXISTANT FONCTIONNERA ---
                        # options=categories_dispo,  <-- Plus d'erreur ici !
                        ...
                        liste_exclusion = virements_tech + categories_a_masquer
                    ...
                    with c_ann:
                        ...
                        with tab_recap:
                            ...
                            df_template = pd.DataFrame({'mois': noms_mois_list})
                            
                            # --- SYNCHRONISATION DES FLUX ---
                            # On utilise df_flux qui a déjà été filtré plus haut dans ton code (Étape 6)
                            if not df_flux.empty:
                                df_reel_mois = df_flux.groupby('mois')['montant'].agg(
                                ...
                                # On utilise directement evolution_mensuelle calculé par la fonction "Source de Vérité"
                                cps_nettoyes = [str(c).strip().upper() for c in cps]
                                df_evolution = pd.DataFrame(evolution_mensuelle)
                                comptes_valides = [c for c in cps_nettoyes if c in df_evolution.columns]

                                if comptes_valides:
                                    df_tab['Patrimoine'] = df_evolution[comptes_valides].sum(axis=1)
                                else:
                                    df_tab['Patrimoine'] = 0.0


                                # --- CALCUL DES TOTAUX ANNUELS ---
                                total_rev = df_tab['Revenus'].sum()
                                total_dep = df_tab['Dépenses'].sum()
                                total_ep  = df_tab['Épargne'].sum()
                                # Pour le patrimoine, on prend généralement la dernière valeur connue de l'année
                                total_patri = df_tab['Patrimoine'].iloc[-1] if not df_tab.empty else 0

                                # --- LIGNE DE TOTAL ANNUEL (FIXE) ---
                                h0,h1, h2, h3, h4, h5,h6 = st.columns([0.1,1.5, 0.8, 1.2, 1, 0.9,0.2])
                                base_h = "margin:0; font-weight:bold; font-size:10px;"
                                
                                h1.markdown(f"<p style='{base_h} text-align:left;'>MOIS</p>", unsafe_allow_html=True)
                                ...
                                h5.markdown(f"<p style='{base_h} text-align:right;'>SOLDE</p>", unsafe_allow_html=True)

                                st.markdown("<div style='margin-top: -10px;'></div>", unsafe_allow_html=True)
                                
                                with st.container(height=345):
                                ...
                                # --- LIGNE DE TOTAL ANNUEL (FIXE) ---
                                t0,t1, t2, t3, t4, t5,t6 = st.columns([0.1,1.5, 0.8, 1.2, 1, 0.8,0.2])
                                base_t = "margin:0; font-weight:bold; font-size:14px; padding: 5px 0;"
                                # Version avec une bordure grise semi-transparente
                                bg_style = "border: 0.5px solid rgba(128, 128, 128, 0.1); border-radius: 5px;"
                                
                                t1.markdown(f"<div style='{bg_style}'><p style='{base_t} text-align:left; color:white; padding-left:5px;'>TOTAL</p></div>", unsafe_allow_html=True)
                                t2.markdown(f"<div style='{bg_style}'><p style='{base_t} text-align:center; color:{col_rev};'>{total_rev:,.0f}€</p></div>", unsafe_allow_html=True)
                                t3.markdown(f"<div style='{bg_style}'><p style='{base_t} text-align:center; color:{col_perf_dep};'>{total_dep:,.0f}€</p></div>", unsafe_allow_html=True)
                                t4.markdown(f"<div style='{bg_style}'><p style='{base_t} text-align:center; color:{col_epargne if total_ep >= 0 else '#ff4b4b'};'>{total_ep:,.0f}€</p></div>", unsafe_allow_html=True)
                            ...
                            with tab_details_cat:
                                if not df_flux.empty:
                                    # 1. On utilise df_flux (déjà filtré) et on ne garde que les montants négatifs (Dépenses)
                                    df_dep = df_flux[df_flux['montant'] < 0].copy()
                                    df_dep['montant'] = df_dep['montant'].abs()

                                    # 2. Création du Pivot (Le reste de ton code pivot est bon)
                    ...
                    with c_graph:
                        ...
                        st.subheader(f"📊 Graphiques et Budgets {annee_choisie}")
                        tab_graph, tab_budgets, tab_projet = st.tabs(["Graphiques", "Budgets","Projets"])
                        with tab_graph:
                            noms_mois = ["Janvier", "Février", "Mars", "Avril", "Mai", "Juin", 
                                        "Juillet", "Août", "Septembre", "Octobre", "Novembre", "Décembre"]

                            # --- 1. APPEL UNIQUE À LA SOURCE DE VÉRITÉ ---
                            # On calcule tout d'un coup : le report au 01/01 et l'évolution mois par mois
                            report_janvier, evolution_mensuelle = calculer_patrimoine_complet(
                                st.session_state.df, 
                                st.session_state.config_groupes, 
                                annee_choisie
                            ...
                            # On exclut les virements '🔄' pour ne pas fausser les revenus/dépenses réels
                            ...
                            # --- 3. CONSTRUCTION DU DATAFRAME DE RÉFÉRENCE (df_tab) ---
                            df_tab = pd.DataFrame({'mois': noms_mois})
                            
                            # On remplit les colonnes de chaque compte avec les données d'évolution
                            for c in cps:
                                nom_c_upper = str(c).strip().upper()
                                if nom_c_upper in evolution_mensuelle:
                                    df_tab[c] = evolution_mensuelle[nom_c_upper]
                                else:
                                    # Si aucune transaction, le solde reste celui du report au 1er janvier
                                    df_tab[c] = report_janvier.get(nom_c_upper, 0.0)

                            # Fusion avec les revenus/dépenses et calculs finaux
                            df_tab = pd.merge(df_tab, df_ann, on='mois', how='left').fillna(0)
                            df_tab['Épargne'] = df_tab['Revenus'] - df_tab['Dépenses']
                            df_tab['Patrimoine'] = df_tab[cps].sum(axis=1)

                            # --- 4. TRACEUR (On coupe après le dernier mois ayant des données) ---
                            ...
                            # --- 5. AFFICHAGE DE LA JAUGE D'ÉPARGNE ---
                            # On calcule l'épargne cumulée sur l'année à partir des flux réels
                            # On utilise df_tab qui contient déjà les colonnes Revenus et Dépenses propres
                            epargne_realisee = df_tab['Épargne'].sum() 

                            # On récupère le solde de départ pour l'affichage de la cible
                            solde_depart_total = sum(report_janvier.values())
                            ...
                            if obj > 0:
                                ...
                                c_t1.markdown(f"**Progression Épargne {annee_choisie}**")
                                
                                # Affichage dynamique (Vert si objectif atteint)
                                color_text = "#27ae60" if epargne_realisee >= obj else "inherit"
                                c_t2.markdown(f"""
                                    <p style='text-align:right; margin:0; font-size:12px;'>
                                        <span style='color:{color_text}; font-weight:bold;'>{epargne_realisee:,.0f}€</span> 
                                        / {obj:,.0f}€ ({prog:.1%})
                                    </p>
                                """, unsafe_allow_html=True)
                                ...
                                st.markdown(f"""
                                    <div style="background:#e0e0e0; border-radius:5px; height:12px; margin-bottom:5px; width:100%;">
                                        <div style="background:{couleur_barre}; width:{prog*100}%; height:12px; border-radius:5px; transition: width 0.5s;"></div>
                                    </div>
                                    <div style="display:flex; justify-content:space-between;">
                                        <p style='font-size:10px; color:gray; margin:0;'>Dépôt initial : {solde_depart_total:,.0f}€</p>
                                        <p style='font-size:10px; color:gray; margin:0;'>Objectif patrimoine : {(solde_depart_total + obj):,.0f}€</p>
                                    </div>
                                """, unsafe_allow_html=True)
        ...
        def afficher_zone_previsionnelle():
            ...
            # --- 3. CALCULS ---
            mois_idx_fin = nomS_mois.index(mois_p) if mois_p != "Tous les mois" else 0
            annee_p_int = int(annee_p)
            cps_upper = [str(c).strip().upper() for c in cps]

            # A. Préparation des DataFrames filtrés
            ...
            # B. Filtrage des prévisions (Yeux ouverts/fermés)
            df_prev_visibles = df_prev_filtre.copy()
            for m, visible in st.session_state.show_prev_mois.items():
                if not visible:
                    df_prev_visibles = df_prev_visibles[df_prev_visibles["mois"] != m]

            # C. APPEL DE LA FONCTION UNIQUE
            df_fusion_pour_calcul = pd.concat([df_reel_filtre, df_prev_visibles], ignore_index=True)
            report_janvier_p, evolution_mensuelle_p = calculer_patrimoine_complet(
                df_fusion_pour_calcul, 
                st.session_state.config_groupes, 
                annee_p_int
            )

            # D. PRÉPARATION DES VARIABLES POUR L'AFFICHAGE
            df_evol_p = pd.DataFrame(evolution_mensuelle_p)
            
            # 1. Solde Global Projeté (pour la Card) -> Fin d'année
            patrimoine_global_projete = df_evol_p[cps_upper].sum(axis=1).iloc[-1] if not df_evol_p.empty else 0.0

            # 2. Solde de base pour le graphique -> Début d'année
            solde_base_annee = sum(report_janvier_p.get(c, 0.0) for c in cps_upper)

            # 3. Données pour le tableau et graphiques (Revenus / Dépenses)
            # On combine réel et prévi visibles uniquement pour l'année en cours
            df_combi = pd.concat([
                df_reel_filtre[df_reel_filtre["année"] == annee_p_int],
                df_prev_visibles[df_prev_visibles["année"] == annee_p_int]
            ], ignore_index=True)

            stats = df_combi.groupby('mois')['montant'].agg(
            ...
            # --- D. PRÉPARATION AFFICHAGE (Version avec filtrage sélectif) ---

            # 1. On crée le combiné Réel + Prévi de l'année
            df_combi = pd.concat([
                df_reel_filtre[df_reel_filtre["année"] == annee_p_int],
                df_prev_filtre[df_prev_filtre["année"] == annee_p_int]
            ], ignore_index=True)
            ...
            # 2. On masque UNIQUEMENT les lignes marquées [PRÉVI] pour les mois où l'œil est fermé
            for m, visible in st.session_state.show_prev_mois.items():
                if not visible:
                    # On ne supprime que les prévisions, on garde le réel du mois m
                    idx_a_supprimer = df_tab_data[
                        (df_tab_data["mois"] == m) & 
                        (df_tab_data["nom"].astype(str).str.contains(r"\[PRÉVI\]", na=False))
                    ].index
                    df_tab_data = df_tab_data.drop(idx_a_supprimer)

            # 3. Calcul des revenus/dépenses pour les graphiques et tableaux
            stats = df_tab_data.groupby('mois')['montant'].agg(
                Rev=lambda x: x[x > 0].sum(), 
                Dep=lambda x: abs(x[x < 0].sum())
            ).reset_index()

            # 4. Reconstruction du patrimoine cumulé pour le graphique prévisionnel
            # On repart de notre solde_base_annee (calculé par la fonction)
            # et on y ajoute l'épargne (Rev - Dep) au fil des mois
            stats['mois_idx'] = stats['mois'].apply(lambda x: nomS_mois.index(x))
            stats = stats.sort_values('mois_idx')
            stats['Epargne'] = stats['Rev'] - stats['Dep']
            stats['Patrimoine_Graph'] = solde_base_annee + stats['Epargne'].cumsum()


            # --- 4. CARDS ---
            st.markdown(f"#### 🏦 Situation Financière projetée {annee_p_int}")
            cols_kpi = st.columns(len(cps) + 1)

            # Card Globale (Somme de tous les comptes du profil au dernier mois)
            with cols_kpi[0]:
                st.markdown(f"""
                    <div style="background-color:{col_patri}; padding:15px; border-radius:12px; text-align:center; color:white;">
                        <p style="margin:0; font-size:12px; font-weight:bold; opacity:0.8;">TOTAL PROJETÉ</p>
                        <p style="margin:0; font-size:20px; font-weight:800;">{patrimoine_global_projete:,.2f} €</p>
                    </div>
                """, unsafe_allow_html=True)

            # Cards individuelles
            for i, c in enumerate(cps):
                nom_c_key = str(c).strip().upper()
                # On récupère la dernière valeur de la liste d'évolution pour ce compte
                solde_final_c = evolution_mensuelle_p[nom_c_key][-1] if nom_c_key in evolution_mensuelle_p else 0.0
                
                couleur = st.session_state.config_groupes.get(c, {}).get("Couleur", "#3498db")
                
                with cols_kpi[i+1]:
                    st.markdown(f"""
                        <div style="background-color:{couleur}; padding:15px; border-radius:12px; text-align:center; color:white;">
                            <p style="margin:0; font-size:11px; font-weight:bold; text-transform:uppercase;">{c}</p>
                            <p style="margin:0; font-size:18px; font-weight:700;">{solde_final_c:,.2f} €</p>
                        </div>
                    """, unsafe_allow_html=True)
            ...
            with col2:
                ...
                # --- 2. FILTRAGE (AJOUT DU FILTRE PROFIL) ---
                # On récupère la liste des comptes du profil actuel (déjà définie plus haut dans ton code)
                cps_upper = [str(c).strip().upper() for c in cps]

                # Filtrage par profil d'abord (Indispensable pour ne pas voir les prévisions d'Aude chez Théo)
                df_source = df_source[df_source['compte'].str.upper().isin(cps_upper)]

                if mois_p == "Tous les mois":
                    # Filtrage par ANNÉE uniquement
                ...
                if not df_mois_prev.empty:
                    ...
                    with st.container(height=470):
                        for idx, r in df_mois_prev.iterrows():
                            ...
                            c_mnt.markdown(f"<p style='color:{color}; font-weight:bold; text-align:right; margin-top:5px; font-size:14px;'>{r['montant']:.2f}€</p>", unsafe_allow_html=True)
                    ...
                    # --- LE MESSAGE SI VIDE ---
                    st.info(f"✨ Aucune prévision trouvée pour **{mois_p} {annee_p}** sur ce profil.")
                    st.write("Vous pouvez en ajouter une nouvelle via le formulaire d'ajout.")
        ...
        # Calcul du point de départ au 1er janvier de l'année sélectionnée pour TOUS les comptes du profil
            ...
            with col3: 
                st.markdown("📊 Récap des prévisions")
                ...
                df_clean = df_tab_data[~mask_interne].copy() # <--- On le définit AVANT les onglets
                ...
                # --- CRÉATION DES TABS ---
                tab_table, tab_chart = st.tabs(["📋 Tableau Annuel", "📈 Par Catégories"])
                with tab_table:

                                            # CSS pour supprimer les marges inutiles des colonnes Streamlit dans le tableau
                    
                                # 4. AFFICHAGE DU TABLEAU
                    st.markdown(f"""<div style="display:flex; font-size:10px; color:gray; font-weight:bold; padding:5px 10px;">
                                    <span style="width:5px;"></span>
                                    <span style="flex:1;text-align:left;">ACTIVER PREVISIONS</span>
                                    <span style="flex:1;text-align:left;">MOIS</span>
                                    <span style="flex:1;text-align:left;">REVENUS</span>
                                    <span style="flex:1;text-align:left;">DÉPENSES</span>
                                    <span style="flex:1;text-align:left;">ÉPARGNE</span>
                                    <span style="flex:1;text-align:center;">SOLDE</span></div>""", unsafe_allow_html=True)
                    ...
                    with st.container(height=450):
                        for _, row in df_tab_p.iterrows():
                            ...
                            bg = "linear-gradient(90deg, rgba(64, 156, 243, 0.2) 0%, rgba(255,255,255,0) 100%)" if m == mois_p else "transparent"
                            ...
                            with c_text:
                                st.markdown(f"""
                                <div style="display:flex; padding:6px 10px; background:{bg}; opacity:{opacity}; border-radius:4px; margin-bottom:2px; font-size:15px; border-bottom:1px solid rgba(128,128,128,0.05);">
                                    <span style="width:60px; font-weight:bold; color:white;text-align:center">{m}</span>
                                    <span style="flex:1;font-size:15px;font-weight:bold; text-align:right; color:{col_rev};">{row['Rev']:,.2f}</span>
                                    <span style="flex:1; font-size:15px;font-weight:bold; text-align:right; color:{col_perf_dep};">{row['Dep']:,.2f}</span>
                                    <span style="flex:1;font-size:15px;font-weight:bold; text-align:right; color:{color_ep}; font-weight:bold;">{row['Epargne']:,.2f}</span>
                                    <span style="flex:1.2; font-size:15px;font-weight:bold; text-align:right; color:{col_patri}; font-weight:bold;">{row['Solde']:,.2f}€</span>
                                </div>""", unsafe_allow_html=True)
                with tab_chart:
                    # --- PRÉPARATION DES DONNÉES FILTRÉES ---
                    # On repart de df_tab_data qui contient déjà le bon Profil, la bonne Année et respecte les "Yeux"
                    df_graph = df_tab_data.copy()

                    # 1. Filtre sur le mois sélectionné (si ce n'est pas "Tous les mois")
                    if mois_p != "Tous les mois":
                        df_graph = df_graph[df_graph["mois"] == mois_p]

                    # 2. On exclut les virements internes pour ne pas fausser les graphiques de dépenses
                    mask_interne = df_graph['categorie'].str.upper().str.contains("🔄|VERS|INTERNE", na=False)
                    df_clean_graph = df_graph[~mask_interne].copy()

                    # 3. Filtrage des dépenses uniquement (montant < 0)
                    df_depenses = df_clean_graph[df_clean_graph['montant'] < 0].copy()
                    
                    if not df_depenses.empty:
                        df_depenses['montant_abs'] = abs(df_depenses['montant'])
                        
                        # Groupement par catégorie
                        cat_stats = df_depenses.groupby('categorie')['montant_abs'].sum().reset_index()
                        cat_stats = cat_stats.sort_values('montant_abs', ascending=True)
                        ...
                        fig_cat = px.bar(
                            ...
                            y='categorie',
                            orientation='h',
                            title=f"Dépenses : {choix_g} • {mois_p} {annee_p}",
                            labels={'montant_abs': 'Total (€)', 'categorie': 'Catégorie'},
                            text='montant_abs',
                        )
                        ...
                        fig_cat.update_layout(
                            ...
                            # Ajustement automatique de la marge gauche pour les longs noms de catégories
                            yaxis=dict(automargin=True)
                        )
                        
                        st.plotly_chart(fig_cat, use_container_width=True)
                    else:
                        st.info(f"🍃 Aucune dépense à afficher pour {mois_p} {annee_p} (Profil: {choix_g}).")
        afficher_zone_previsionnelle()
                
    elif selected == "comptes":
            st.subheader("👥 Structure & comptes")
            st.markdown("Organisez vos finances par groupes et configurez vos soldes de départ.")
           ...
           # --- POPOVER BLOC-NOTES ---
                        ...
                        def valider_et_nettoyer():
                            ...
                            if nom:
                                ...
                                if sauvegarder_nouvelle_categorie_neon(val_finale, st.session_state.user):
                                    ...
                                    # ON INDIQUE QU'ON VIENT DE CRÉER UNE CATÉGORIE
                                    st.session_state["juste_cree"] = True 
                            ...
                            st.session_state.input_new_cat = ""
                        with st.container(border=True):
                            label_popover = f"Icône : {st.session_state.get('emoji_choisi', '📁')}"
                            with st.popover(label_popover, width='stretch'):
                                # --- AJOUT : Champ de saisie libre ---
                                col_custom, col_btn = st.columns([3, 1], vertical_alignment="bottom")
                                #st.markdown("raccourcis clavier pour afficher les emojis: win+;")
                                # On écrit DIRECTEMENT dans col_custom
                                col_custom.markdown("Emoji perso")
                                col_custom.caption("⌨️ Raccourcis clavier : win + ;")
                                custom_emoji = col_custom.text_input(
                                    label="emoji", 
                                    placeholder="Copiez ici...", 
                                    key="input_custom_emoji", 
                                    label_visibility="collapsed" # On cache le label du widget car on a mis notre markdown au dessus
                                )
                                
                                
                                if col_btn.button("OK"):
                                    if custom_emoji:
                                        # On nettoie et on force la variante couleur
                                        emoji_nettoye = custom_emoji.strip()[0]
                                        st.session_state.emoji_choisi = f"{emoji_nettoye}\uFE0F"
                                        st.rerun(scope="fragment")                         
                                ...
                                for idx, (cat_icon, icons) in enumerate(categories.items()):
                                    with tabs[idx]:
                                        ...
                                        for i, icon in enumerate(icons):
                                            # On ajoute \uFE0F à l'icône pour forcer le rendu "Emoji" coloré
                                            full_icon = f"{icon}\uFE0F" 
                                            
                                            if cols[i % 4].button(full_icon, key=f"emo_{idx}_{i}", use_container_width=True):
                                                st.session_state.emoji_choisi = full_icon # On stocke la version forcée
                                                st.rerun(scope="fragment")
            ...
            with col_main:  
                ...
                def zone_interactive_tableau():
                    ...
                    annee_actuelle = str(datetime.datetime.now().year)
                    mois_actuel = nomS_mois[datetime.datetime.now().month - 1] # nomS_mois doit être défini

                    # Initialisation du groupe par défaut (le 1er de la liste si disponible)
                    if "filter_g" not in st.session_state:
                        st.session_state.filter_g = st.session_state.groupes_liste[0] if st.session_state.groupes_liste else "Tous"

                    if "filter_c" not in st.session_state:
                    ...
                    with col_sidebar:   
                        ...
                        with st.container(border=True):
                            ...
                            def reset_change_flag():
                                # Cette fonction va dire au popover : 
                                # "Tout va bien, c'est juste un changement de vue, pas une modif de donnée"
                                st.session_state["juste_cree"] = True
                            ...
                            # On force l'index sur le 1er groupe réel (index 1) au lieu de "Tous" (index 0) si on arrive
                            idx_g_default = 1 if len(liste_g) > 1 else 0
                            
                            new_g = st.selectbox("Groupe", liste_g, 
                                                index=liste_g.index(st.session_state.filter_g) if st.session_state.filter_g in liste_g else idx_g_default,
                                                key="frag_filter_g",on_change=reset_change_flag)
                            ...
                            new_c = st.selectbox("Compte", cps_filtre, 
                                                ...
                                                key="frag_filter_c",on_change=reset_change_flag)
                            st.session_state.filter_c = new_c

                            # Années et Mois
                            # --- FILTRE ANNÉE ---
                            liste_a = ["Toutes"] + sorted([str(int(a)) for a in df_f['année'].dropna().unique()], reverse=True)
                            # Calcul de l'index pour l'année en cours
                            idx_a = liste_a.index(annee_actuelle) if annee_actuelle in liste_a else 0
                            
                            annee_selectionnee = st.selectbox("Année", liste_a, index=idx_a, key="filter_a_select",on_change=reset_change_flag)
                            ...
                            idx_m = liste_m.index(mois_actuel) if mois_actuel in liste_m else 0
                            st.selectbox("Mois", liste_m, index=idx_m, key="filter_m_select",on_change=reset_change_flag)
                        ...
                        # --- LOGIQUE DE TRI ---
                            ...
                            sort_label = st.selectbox("Trier par", list(map_sort.keys()), index=0, label_visibility="collapsed", key="sort_choice",on_change=reset_change_flag)
                            st.session_state.sort_by = map_sort[sort_label]
                            st.session_state.sort_order = st.selectbox("Ordre", ["↑ Croissant", "↓ Décroissant"], index=1, label_visibility="collapsed", key="sort_ord",on_change=reset_change_flag)
                                        ...
                                        with st.spinner("Suppression dans Neon..."):
                                            ...
                                            if success:
                                                ...
                                                for idx in indices_selectionnes:
                                                    if f"cb_v3_{idx}" in st.session_state:
                                                        del st.session_state[f"cb_v3_{idx}"]
                        ...
                        def afficher_tableau_transactions(df_f):
                            ...
                            with st.container(height=570, border=True,):
                                ...
                                for idx, row in df_f.iterrows():
                                    ...
                                    c_info,c_nom, c_cat, c_mois,c_split, c_del = st.columns([2,0.5, 1.8, 1.5,0.5, 0.5])

                                    st.markdown("""
                                        <style>
                                            .modified-gradient {
                                                background: linear-gradient(90deg, rgb(64, 156, 243,0.3) 0%, rgba(255,255,255,0) 100%);
                                                
                                            }
                                        </style>
                                    """, unsafe_allow_html=True)
                                    
                                    # --- DANS TA BOUCLE D'AFFICHAGE (afficher_tableau_transactions) ---

                                    # 1. On récupère le témoin (comme dans ton popover)
                                    temoin = st.session_state.get("df_temoin", {})

                                    # 2. On définit les mêmes clés et IDs stables
                                    id_stable = f"{row['nom']}_{row['date']}_{row['montant']}"
                                    k_nom = f"nom_{id_stable}_{idx}"
                                    k_mo = f"mo_{idx}" 
                                    k_cat = f"cat_{id_stable}_{idx}"

                                    # 3. Récupération intelligente
                                    # Si la session est vide, on prend ce qui est affiché dans la ligne actuelle
                                    val_session_nom = st.session_state.get(k_nom, row['nom']) # On prend le nom actuel par défaut
                                    val_session_mo = st.session_state.get(k_mo, row['mois'])
                                    val_session_cat = st.session_state.get(k_cat, row['categorie'])

                                    # 4. Récupération du témoin (point de référence à l'import)
                                    cat_init = temoin.get(idx, row['categorie'])
                                    mo_init = row['mois']

                                    change_cat = val_session_cat is not None and str(val_session_cat) != str(cat_init)
                                    change_mo = val_session_mo is not None and str(val_session_mo) != str(mo_init)
                                    change_nom = str(val_session_nom) != str(row['nom'])

                                    is_modified = change_mo or change_cat or change_nom
                                    ...
                                    with c_info:
                                        # --- STYLE ---
                                        # La classe s'applique si l'un des deux a changé en session
                                        div_class = "modified-gradient" if is_modified else ""

                                        
                                        
                                        st.markdown(f'''
                                            <div class="{div_class}" style="border-left:3px solid {color_amount}; padding-left:8px; line-height:1.2; padding-top:2px; padding-bottom:2px;">
                            ...
                            with st.container(border=True):
                                # On ajuste les colonnes pour faire de la place au nouveau bouton
                                c_save0, c_save1, c_save2 = st.columns([1, 1.5, 1.5],vertical_alignment= "top")
                                
                                
                                with c_save1:
                                    apprendre = st.toggle("🧠 Mémoriser les catégories modifiées", value=False,help="Active l'apprentissage automatique : si vous renommez une catégorie, toutes les autres transactions portant exactement le même nom seront mises à jour avec cette nouvelle catégorie.")
                                ...
                                with c_save0:
                                    ...
                                    # 1. On détermine si on doit scanner les changements
                                    if st.session_state.get("juste_cree"):
                                        st.session_state["juste_cree"] = False 
                                        liste_recap = [] 
                                    else:
                                        for idx_f in df_f.index:
                                            row_f = df_f.loc[idx_f]
                                            
                                            # 1. Identifiants et Clés
                                            id_stable_f = f"{row_f['nom']}_{row_f['date']}_{row_f['montant']}"
                                            k_mo = f"mo_{idx_f}"
                                            k_cat = f"cat_{id_stable_f}_{idx_f}"
                                            k_nom = f"nom_{id_stable_f}_{idx_f}" # Clé pour le nom
                                            
                                            # 2. Valeurs de référence (Import initial)
                                            nom_init = row_f['nom'] 
                                            cat_init = temoin.get(idx_f)
                                            mo_init = row_f['mois']

                                            # 3. Récupération des valeurs en session (Modifications)
                                            val_session_mo = st.session_state.get(k_mo, mo_init)
                                            val_session_cat = st.session_state.get(k_cat, cat_init)
                                            val_session_nom = st.session_state.get(k_nom, nom_init)

                                            # 4. Détection des changements
                                            change_mo = str(val_session_mo) != str(mo_init)
                                            change_cat = val_session_cat is not None and str(val_session_cat) != str(cat_init)
                                            change_nom = str(val_session_nom) != str(nom_init)

                                            # --- CORRECTION DE LA CONDITION ---
                                            if change_cat or change_mo or change_nom:
                                                liste_recap.append({
                                                    "nom_old": nom_init,
                                                    "nom_new": val_session_nom,
                                                    "n_ch": change_nom,
                                                    "cat_old": cat_init, 
                                                    "cat_new": val_session_cat, 
                                                    "c_ch": change_cat,
                                                    "mo_old": mo_init, 
                                                    "mo_new": val_session_mo, 
                                                    "m_ch": change_mo
                                                })

                                    label = f"Prévisualiser les changements ({len(liste_recap)})"
                                    ...
                                    with st.popover(label, use_container_width=True):
                                        if liste_recap:
                                            if apprendre:
                                                col_tab, col_app = st.columns(2)
                                            else:
                                                col_tab = st.container()

                                            # --- COLONNE 1 : CHANGEMENTS TABLEAU ---
                                            with col_tab:
                                                st.markdown("📃 **Changements tableau**")
                                                
                                                # On construit tout le HTML dans une variable
                                                html_tableau = ""
                                                for item in liste_recap:
                                                    # 1. On définit le nom pour cette itération
                                                    if item['n_ch']:
                                                        nom_visuel = f"<span style='color:#f34c40;'>{item['nom_old']}</span> ➡️ {item['nom_new']}"
                                                    else:
                                                        nom_visuel = item['nom_old']
                                                    
                                                    # 2. On définit les lignes de changement
                                                    cat_part = f"<div style='margin-bottom:2px;'>❌ <span style='color:#f34c40;font-weight:bold;'>{item['cat_old']}</span> ➡️ <span style='color:white;font-weight:bold;'>{item['cat_new']}</span></div>" if item['c_ch'] else ""
                                                    mo_part = f"<div>❌ 📆 <span style='color:#f34c40;font-weight:bold;'>{item['mo_old']}</span> ➡️ <span style='color:white;font-weight:bold;'>📆 {item['mo_new']}</span></div>" if item['m_ch'] else ""

                                                    # 3. On ajoute la carte au bloc
                                                    html_tableau += f"""
                                                    <div style="margin-bottom:12px;padding:8px;border-left:4px solid #5092d8;background: linear-gradient(90deg, rgba(14, 156, 243, 0.2) 0%, rgba(255,255,255,0) 100%);line-height:1.2;">
                                                        <div style="font-weight:600;color:white;margin-bottom:4px;">{nom_visuel}</div>
                                                        {cat_part}{mo_part}
                                                    </div>"""
                                                
                                                st.markdown(html_tableau, unsafe_allow_html=True)
                                                
                                            # --- COLONNE 2 : APPRENTISSAGE ---
                                            if apprendre:
                                                with col_app:
                                                    ...
                                                    for item in liste_recap:
                                                        # On apprend si la catégorie a changé
                                                        if item['c_ch']:
                                                            # On utilise le NOUVEAU NOM comme référence pour l'apprentissage si il a changé
                                                            nom_pour_cle = item['nom_new'] if item['n_ch'] else item['nom_old']
                                                            
                                                            if nom_pour_cle not in noms_deja_vus:
                                                                # On recrée un affichage propre pour cette colonne
                                                                nom_visuel = item['nom_new'] if item['n_ch'] else item['nom_old']
                                                                
                                                                app_html += f"""
                                                                <div style="margin-bottom: 12px; padding: 4px 0 4px 12px; border-left: 4px solid #a29bfe; background: linear-gradient(90deg, rgba(162, 155, 254, 0.2) 0%, rgba(255,255,255,0) 100%); line-height: 1.2;">
                                                                    <div style="font-weight: 600; color: #a29bfe; font-size: 0.9rem; margin-bottom: 4px;">{nom_visuel[:25]}</div>
                                                                    <div style="color: white; font-size: 0.9rem; font-weight:bold;">🎯 Appris : <b>{item['cat_new']}</b></div>
                                                                </div>
                                                                """
                                                                noms_deja_vus.add(nom_pour_cle)
                                                    
                                                    if app_html:
                                                        st.markdown(app_html, unsafe_allow_html=True)
                                                    else:
                                                        st.caption("Aucune règle à apprendre.")
                                        else:
                                ...
                                with c_save2:
                                    if st.button("💾 Sauvegarder les modifications", type="primary", use_container_width=True):
                                        ...
                                        # --- ÉTAPE CRUCIALE : On sécurise le DF principal ---
                                        # On s'assure que la colonne 'nom' est bien du texte pur
                                        st.session_state.df['nom'] = st.session_state.df['nom'].astype(str).str.strip()

                                        # --- 1. APPLICATION DES MODIFICATIONS ---
                                        for idx_f in df_f.index:
                                            row_f = df_f.loc[idx_f]
                                            
                                            # On force tout en string pour la création de la clé
                                            nom_actuel_brut = str(row_f['nom']).strip()
                                            date_brute = str(row_f['date'])
                                            montant_brut = str(row_f['montant'])
                                            
                                            id_stable_f = f"{nom_actuel_brut}_{date_brute}_{montant_brut}"
                                            
                                            key_nom = f"nom_{id_stable_f}_{idx_f}"
                                            key_cat = f"cat_{id_stable_f}_{idx_f}"
                                            key_mo = f"mo_{idx_f}"

                                            nom_pour_apprentissage = nom_actuel_brut

                                            # --- A. Mise à jour du NOM ---
                                            if key_nom in st.session_state:
                                                nouveau_nom = str(st.session_state[key_nom]).strip()
                                                if nouveau_nom != nom_actuel_brut:
                                                    dict_noms_originaux[idx_f] = nom_actuel_brut
                                                    st.session_state.df.at[idx_f, 'nom'] = nouveau_nom
                                                    nom_pour_apprentissage = nouveau_nom

                                            # --- B. Mise à jour du MOIS ---
                                            if key_mo in st.session_state:
                                                st.session_state.df.at[idx_f, 'mois'] = str(st.session_state[key_mo])

                                            # --- C. Logique d'apprentissage ---
                                            if key_cat in st.session_state:
                                                cat_choisie = str(st.session_state[key_cat])
                                                cat_initiale = str(temoin.get(idx_f, ""))

                                                if cat_choisie != cat_initiale:
                                                    if apprendre:
                                                        nouvelles_regles.append((nom_pour_apprentissage, cat_choisie))
                                                        # Le masque est maintenant sécurisé car tout est en String
                                                        mask = st.session_state.df['nom'] == nom_pour_apprentissage
                                        ...
                                        try:
                                            ...
                                            with st.spinner("Sauvegarde en cours..."):
                                                success = sauvegarder_donnees_neon(df_a_sauver, user_actuel, dict_noms_originaux=dict_noms_originaux)
                                                if success:
                                                    # --- NETTOYAGE COMPLET DE LA SESSION ---
                                                    # On ajoute k.startswith("nom_") ici !
                                                    cles_a_supprimer = [k for k in st.session_state.keys() if 
                                                                    k.startswith("cat_") or 
                                                                    k.startswith("mo_") or 
                                                                    k.startswith("nom_")]
                                                    ...
                                                    st.cache_data.clear()
                                                    st.success("✅ Modifications (noms, dates, catégories) enregistrées !")
                                                    time.sleep(1)
                                                    st.rerun()
                                        except Exception as e:
    ...
    # --- LOGIQUE DE NETTOYAGE SÉCURISÉE ---

    # 1. Initialisation si c'est la première fois qu'on ouvre l'app
    if "last_page" not in st.session_state:
        st.session_state.last_page = selected

    # 2. On vérifie si la page a RÉELLEMENT changé
    if st.session_state.last_page != selected:
        # On fait le ménage
        for key in list(st.session_state.keys()):
            if key.startswith("cat_") or key.startswith("mo_") or key == "df_temoin":
                del st.session_state[key]
        
        # CRUCIAL : On met à jour l'indicateur AVANT le rerun
        st.session_state.last_page = selected
        ...
        # --- TAB IMPORT (VERSION CORRIGÉE ET SÉCURISÉE) ---
            ...
            st.markdown("Glissez votre relevé bancaire au format CSV pour synchroniser vos comptes")
            ...
            with col_upload:
                ...
                if f:
                    ...
                    # --- AJOUT : APERÇU RAPIDE ---
                    with st.popover("🔍 Voir le contenu complet du CSV", use_container_width=True):
                        try:
                            # On remet le pointeur au début pour lire le fichier
                            f.seek(0)
                            # Décodage avec gestion d'erreur pour éviter le crash sur des caractères spéciaux
                            raw_content = f.read()
                            ...
                            if content:
                                ...
                                # Détection de l'en-tête (logique identique à la tienne)
                                header_line_idx = 0
                                for i, line in enumerate(lines[:20]): # On check les 20 premières lignes
                                    l_low = line.lower()
                                    if "date" in l_low and any(m in l_low for m in ["montant", "debit", "credit", "libellé", "nom"]):
                                        header_line_idx = i
                                        break
                                ...
                                # Affichage dans le popover
                                st.write(f"**Nombre de transactions détectées :** {len(df_preview)}")
                                st.dataframe(df_preview, use_container_width=True)
                                
                                # IMPORTANT : On remet le pointeur à 0 pour que la suite de ton code d'importation puisse relire le fichier
                                f.seek(0)
                                
                        except Exception as e_prev:
                            st.warning(f"Impossible d'afficher l'aperçu : {e_prev}")
                            # Sécurité : on remet à 0 quoi qu'il arrive
                            f.seek(0)
                col_imp, col_help = st.columns([0.4, 1],vertical_alignment="bottom")
                with col_imp:
                    st.markdown("<br>", unsafe_allow_html=True)
                ...
                with st.container(border=True):
                    st.markdown("🛠️ **Configuration des mots-clés pour la catégorisation automatique**")
                    
                    # On charge les données (Assure-toi que ces fonctions sont décorées avec @st.cache_data)
                    dict_auto = charger_dictionnaire_mots_cles()
                    all_cats = charger_categories_neon(st.session_state.get("user"))

                    # AJOUT D'UNE KEY FIXE : Cela évite que Streamlit perde le focus
                    cat_to_edit = st.selectbox(
                        "Sélectionnez une catégorie à configurer", 
                        all_cats,
                        key="sb_select_cat_config" 
                    )

                    # 2. Formulaire avec une clé qui dépend de la catégorie
                    # C'est normal que le formulaire change d'ID quand la catégorie change, 
                    # car le contenu doit être totalement remplacé.
                    with st.form(key=f"form_mots_{cat_to_edit}", clear_on_submit=False):
                        mots_actuels = dict_auto.get(cat_to_edit, [])
                        
                        col1, col2 = st.columns([2, 1], vertical_alignment="bottom")
                        
                        # Multiselect : on utilise une clé stable aussi
                        nouveaux_mots = col1.multiselect(
                            f"Mots-clés pour {cat_to_edit}",
                            options=list(set(mots_actuels + st.session_state.get("temp_mots", []))), 
                            default=mots_actuels,
                            key=f"ms_{cat_to_edit}"
                        )
                        
                        nouveau_m = col2.text_input("➕ Ajouter", placeholder="Ex: NETFLIX", key=f"ti_{cat_to_edit}").upper()
                        
                        submit = st.form_submit_button("Enregistrer", type="primary", use_container_width=True)
                        
                        if submit:
                            mots_finaux = nouveaux_mots 
                            if nouveau_m and nouveau_m.strip() != "":
                                if nouveau_m not in mots_finaux:
                                    mots_finaux.append(nouveau_m.strip())
                            
                            if update_mots_cles_neon(cat_to_edit, mots_finaux):
                                st.success("✅ Mis à jour !")
                                charger_dictionnaire_mots_cles.clear() 
                                st.rerun()
                                                    ...
                                                    with ce1:
                                                        ...
                                                        with st.popover(emoji_db):
                                                            ...
                                                            for i_e, e in enumerate(choix):
                                                                if cols_pop[i_e % 5].button(e, key=f"sql_emo_{p}_{i_e}"):
                                                                    try:
                                                                        with engine.begin() as conn:

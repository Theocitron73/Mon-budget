...
# ---------------------------------------------------
        ...
        with col1:
            ...
            with st.form("bulk_add_form_v4"):
                ...
                submit = st.form_submit_button("Enregistrer les prévisions 💾", use_container_width=True, type="primary")
            ...
            with st.container(height=480):
                if not df_mois_prev.empty:
                    for idx, r in df_mois_prev.sort_values("Date").iterrows():
                        ...
                        if c3.button("", icon=":material/delete:", key=f"del_{idx}", use_container_width=True):
            ...
            with st.container(height=280):
                for _, row in df_tab_p.iterrows():
                    ...
                    with c_eye:
                        ...
                        if st.button(" ", key=f"btn_{m}", help=f"Afficher/Masquer {m}",use_container_width=True):
    ...
    # Popover compact avec onglets
                        ...
                        # Bouton de création
                        st.button("Créer la catégorie ✨", use_container_width=True, type="primary", on_click=valider_et_nettoyer)

                fragment_categorie()

                # 2. BOUTON D'ACTUALISATION (Hors fragment)
                # Ce bouton permet de voir les nouvelles catégories dans le tableau de droite
                st.button("🔄 Actualiser les catégories", use_container_width=True, help="Cliquez pour mettre à jour les listes du tableau après une création")
            ...
            with col_add:
                ...
                with st.form("form_reel_multi"):
                    ...
                    for idx in st.session_state.indices_reel:
                        with st.container():
                            ...
                            ops_reelles.append({
                                "Date": pd.to_datetime(f_date), "Nom": f_nom, "Montant": f_mnt,
                                "Categorie": f_cat, "Compte": f_compte,
                                "Mois": NOMS_MOIS[f_date.month - 1], "Année": f_date.year,
                                "User": st.session_state["username"]
                            })
                            st.markdown("<hr style='margin:10px 0; opacity:0.1;'>", unsafe_allow_html=True)

                    submit_reel = st.form_submit_button("Enregistrer les opérations 🚀", use_container_width=True, type="primary")
                ...
                # 1. On définit la fonction du Popup (Dialogue) en haut de votre script ou avant le bloc "Actions"
                ...
                def confirmer_suppression_dialog(nb_lignes, df_a_supprimer):
                    ...
                    with col1:
                        if st.button("Annuler", use_container_width=True):
                            st.rerun() # Ferme le popup simplement
            ...
            with col_main:
                ...
                if not df_f.empty:
                    ...
                    # --- LOGIQUE DE TRI ---
                    c_head1, c_head2, c_head3,c_head4 = st.columns([1.5, 1.5,1.6,1.8])
                    with c_head1:

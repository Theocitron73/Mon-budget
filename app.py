                        ...
                        def afficher_tableau_transactions(df_f):
                            ...
                            with st.container(height=570, border=True,):
                                ...
                                for idx, row in df_f.iterrows():
                                    ...
                                    is_modified = change_mo or change_cat
                            ...
                            with st.container(border=True):
                                ...
                                with c_save0:
                                    ...
                                    for idx_f in df_f.index:
                                        ...
                                        # On récupère la valeur en session. 
                                        # Si c'est None (pas encore touché), on considère que c'est la valeur actuelle du DF
                                        val_session_mo = st.session_state.get(k_mo, row['mois'])
                                        val_session_cat = st.session_state.get(k_cat)
                                        
                                        # On récupère la référence (ce qu'il y avait juste après l'import)
                                        cat_init = temoin.get(idx_f)
                                        mo_init = row_f['mois'] # Pour le mois, la réf est la colonne 'mois' d'origine
                                    ...
                                    with st.popover(label, use_container_width=True):
                                        if liste_recap:
                                            ...
                                            with col_tab:
                                                ...
                                                # On initialise avec une div ouvrante
                                                html_final = '<div style="display: block;">'
                                                ...
                                                for item in liste_recap:
                                                    ...
                                                    if item['c_ch']:
                                                        # On met tout sur une seule ligne pour éviter que Streamlit ne casse le rendu
                                                        cat_part = f"<div style='margin-bottom:2px;'>❌ <span style='color:#f34c40;font-weight:bold;'>{item['cat_old']}</span> ➡️ <span style='color:white;font-weight:bold;'>{item['cat_new']}</span></div>"
                                                    
                                                    if item['m_ch']:
                                                        mo_part = f"<div>❌ 📆 <span style='color:#f34c40;font-weight:bold;'>{item['mo_old']}</span> ➡️ <span style='color:white;font-weight:bold;'>📆 {item['mo_new']}</span></div>"

                                                    # On assemble la "carte" sans sauts de lignes excessifs
                                                    html_final += f"""<div style="margin-bottom:12px;padding:8px;border-left:4px solid #5092d8;background: linear-gradient(90deg, rgba(14, 156, 243, 0.2) 0%, rgba(255,255,255,0) 100%);line-height:1.2;">
                                                        <div style="font-weight:600;color:white;margin-bottom:4px;">{item['nom'][:15]}</div>
                                                        {cat_part}{mo_part}
                                                    </div>"""
                                                ...
                                                # L'astuce ultime : On utilise st.components.v1.html OU st.markdown avec un nettoyage
                                                st.markdown(html_final, unsafe_allow_html=True)
                                                
                                            if apprendre:
                                                with col_app:
                                                    st.markdown("🧠 **Apprentissage**")
                                                    # On ne montre que les changements de catégorie (l'apprentissage ne gère pas les mois)
                                                    app_html = ""
                                                    noms_deja_vus = set()
                                                    
                                                    for item in liste_recap:
                                                        if item['c_ch'] and item['nom'] not in noms_deja_vus:
                                                            app_html += f"""
                                                            <div style="margin-bottom: 12px; padding: 4px 0 4px 12px; border-left: 4px solid #a29bfe; background: linear-gradient(90deg, rgba(162, 155, 254, 0.2) 0%, rgba(255,255,255,0) 100%); line-height: 1.2;">
                                                                <div style="font-weight: 600; color: #a29bfe; font-size: 0.9rem;font-weight:bold; margin-bottom: 4px;">{item['nom'][:25]}</div>
                                                                <div style="color: white; font-size: 0.9rem;font-weight:bold;">🎯 Appris : <b>{item['cat_new']}</b></div>
                                                            </div>
                                                            """
                                                            noms_deja_vus.add(item['nom'])
                                        ...
                                        try:
                                            ...
                                            with st.spinner("Sauvegarde en cours..."):
                                                ...
                                                if success:
                                                    ...
                                                    cles_a_supprimer = [k for k in st.session_state.keys() if k.startswith("cat_") or k.startswith("mo_")]
                                                    for k in cles_a_supprimer:
                                                        del st.session_state[k]
                                                    
                                                    # On supprime aussi le témoin pour qu'il soit recréé proprement au prochain tour
                                                    if "df_temoin" in st.session_state:
                                                        del st.session_state.df_temoin
                                                    st.cache_data.clear()
        ...
        # --- TAB IMPORT (VERSION CORRIGÉE ET SÉCURISÉE) ---
            ...
            with col_upload:
                ...
                with st.container(border=True):
                    ...
                    # 1. On charge les données (hors du formulaire pour que la selectbox fonctionne)
                    dict_auto = charger_dictionnaire_mots_cles()
                    all_cats = charger_categories_neon(st.session_state.get("user"))

                    # Selectbox HORS formulaire pour que le contenu du formulaire 
                    # s'adapte dès qu'on change de catégorie
                    cat_to_edit = st.selectbox("Sélectionnez une catégorie à configurer", all_cats)

                    # 2. Début du Formulaire pour les modifications
                    with st.form(key=f"form_mots_{cat_to_edit}", clear_on_submit=False):
                        mots_actuels = dict_auto.get(cat_to_edit, [])
                        
                        col1, col2 = st.columns([2, 1], vertical_alignment="bottom")
                        
                        # Multiselect pour gérer l'existant
                        nouveaux_mots = col1.multiselect(
                            f"Mots-clés actuels pour {cat_to_edit}",
                            options=mots_actuels + st.session_state.get("temp_mots", []), # On fusionne
                            default=mots_actuels,
                            help="Supprimez des mots en cliquant sur la croix."
                        )
                        
                        # Input pour le nouveau mot
                        nouveau_m = col2.text_input("➕ Ajouter un mot", placeholder="Ex: NETFLIX").upper()
                        
                        # Bouton de soumission unique
                        submit = st.form_submit_button("Enregistrer les modifications", type="primary", use_container_width=True)
                        
                        if submit:
                            # 1. On part de ce qui est resté sélectionné dans le multiselect
                            # (Si l'utilisateur a cliqué sur la croix, le mot n'est plus dans cette liste)
                            mots_finaux = nouveaux_mots 
                            
                            # 2. On ajoute le nouveau mot s'il y en a un de tapé
                            if nouveau_m and nouveau_m.strip() != "":
                                if nouveau_m not in mots_finaux:
                                    mots_finaux.append(nouveau_m.strip())
                            
                            # 3. On met à jour la base de données avec la liste propre
                            if update_mots_cles_neon(cat_to_edit, mots_finaux):
                                st.success(f"✅ Liste mise à jour pour {cat_to_edit} !")
                                # On vide le cache pour que la catégorisation automatique 
                                # prenne en compte les suppressions immédiatement

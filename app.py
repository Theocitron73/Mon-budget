...
def generer_pdf_tricount(nom_groupe, df_groupe, transferts, total):
    pdf = FPDF()
    pdf.set_auto_page_break(auto=True, margin=15)
    pdf.add_page()
    
    # Calcul de la largeur disponible (W)
    # Largeur de la page A4 (210mm) - marges (10mm gauche + 10mm droite) = 190mm
    W = 190 

    # --- Titre ---
    pdf.set_font("Helvetica", "B", 20)
    pdf.cell(W, 10, f"Resume Tricount : {nom_groupe}", ln=True, align='C')
    
    pdf.set_font("Helvetica", "", 12)
    pdf.cell(W, 10, f"Total des depenses : {total:.2f} EUR", ln=True, align='C')
    pdf.ln(10)

    # --- Historique (Tableau) ---
    pdf.set_font("Helvetica", "B", 12)
    pdf.cell(W, 10, "Detail des depenses :", ln=True)
    pdf.set_font("Helvetica", "", 10)
    
    # Largeurs colonnes : 30 + 70 + 50 + 40 = 190
    ...
    for _, row in df_groupe.iterrows():
        if row['Montant'] > 0:
            ...
            # On tronque le libellé s'il est trop long pour éviter de casser le tableau
            libelle_propre = str(row['Libellé'])[:35] 
            pdf.cell(70, 8, libelle_propre, border=1)
            pdf.cell(50, 8, str(row['Payé_Par']), border=1)
            pdf.cell(40, 8, f"{row['Montant']:.2f} EUR", border=1, ln=True)
    
    pdf.ln(10)

    # --- Remboursements ---
    pdf.set_font("Helvetica", "B", 12)
    pdf.cell(W, 10, "Remboursements a effectuer :", ln=True)
    pdf.set_font("Helvetica", "", 11)
    
    for t in transferts:
        texte = f"- {t['de']} doit donner {t['montant']:.2f} EUR a {t['a']}"
        
        # 1. On s'assure que le curseur est bien à la marge gauche (x=10)
        pdf.set_x(10) 
        
        # 2. On écrit la ligne. multi_cell avec ln=True ou simplement cell si c'est court
        # Ici multi_cell est bien si le nom est très long et doit passer à la ligne
        pdf.multi_cell(W, 8, texte, align='L')
        
        # 3. Petit espace vertical entre chaque ligne de remboursement
        pdf.ln(2)
...
# Si l'utilisateur EST connecté
   ...
   # Encore plus petit et discret
    ...
    selected = option_menu(
        menu_title=None,
        options=["Analyses", "Prévisionnel", "Gestion", "Import", "Comptes", "Tricount"],
    ...
    index_actuel = ["Analyses", "Prévisionnel", "Gestion", "Import", "Comptes","Tricount"].index(selected)
    ...
    # Popover compact avec onglets
        ...
        # On charge ici ta liste par défaut si vide
            ...
            with c_save1:
                # Sauvegarde globale
                apprendre = st.checkbox("🧠 Mémoriser les catégories modifiés", value=False,help="Active l'apprentissage automatique : si vous renommez une catégorie, toutes les autres transactions portant exactement le même nom seront mises à jour avec cette nouvelle catégorie.")
            with c_save2:
        ...
        if groupe_choisi == "-- Choisir un groupe --":
            ...
            col_membres, col_saisie, col_bilan, col_histo = st.columns([0.8, 1.2, 1.2, 1.8], gap="medium")
            ...
            with col_saisie:
                ...
                with st.form("form_tricount_custom", border=True):
                    ...
                    parts_saisies = {}
                    # On affiche les inputs de part de manière compacte
                    for p in st.session_state.participants:
                        parts_saisies[p] = st.number_input(f"Part de {p}", min_value=0.0, max_value=montant_total, step=0.5, key=f"part_{p}")

                    somme_parts = sum(parts_saisies.values())
                    
                    if somme_parts != montant_total and montant_total > 0:
                        st.warning(f"Total: {somme_parts} / {montant_total}€")
                    
                    submit_t = st.form_submit_button("Enregistrer 💾", use_container_width=True)

                if submit_t and montant_total > 0 and abs(somme_parts - montant_total) < 0.01:
                    repartition_str = ",".join([f"{p}:{val}" for p, val in parts_saisies.items() if val > 0])
                    nouvelle_depense = {
                        "Date": datetime.now().strftime("%d/%m/%Y"),
                        "Libellé": libelle,
                        "Payé_Par": payeur,
            ...
            with col_bilan:
                ...
                if not df_tri.empty:
                    ...
                    for _, row in df_groupe.iterrows():
                        payeur_row = row['Payé_Par']
                        mt = float(row['Montant'])
                        
                        # SÉCURITÉ : Si le payeur n'est pas dans la liste, on l'ajoute à la volée
                        if payeur_row not in balances:
                            balances[payeur_row] = 0.0
                        
                        balances[payeur_row] += mt
                        
                        # Décodage des parts spécifiques Nom:Valeur
                        for part_data in str(row['Pour_Qui']).split(','):
                            if ':' in part_data:
                                nom_b, val_b = part_data.split(':')
                                
                                # SÉCURITÉ : Idem pour les bénéficiaires
                                if nom_b not in balances:
                                    balances[nom_b] = 0.0
                                    
                                balances[nom_b] -= float(val_b)

                for personne, solde in balances.items():
                    if solde > 0.01:
                        st.success(f"**{personne}** : +{solde:.2f}€")
                    elif solde < -0.01:
                        st.error(f"**{personne}** : {solde:.2f}€")
                    else:
                        st.write(f"⚪ {personne} : 0.00€")
                ...
                # Définition des listes (Crucial)
                positives = sorted([[p, s] for p, s in balances.items() if s > 0.01], key=lambda x: x[1], reverse=True)
                negatives = sorted([[p, s] for p, s in balances.items() if s < -0.01], key=lambda x: x[1])

                if not positives and not negatives:
                    st.info("✨ Tout le monde est quitte !")
                else:
                    # 1. CALCUL DES TRANSFERTS
                    pos_calc = [list(p) for p in positives]
                    neg_calc = [list(n) for n in negatives]
                    transferts = []

                    for neg in neg_calc:
                        for pos in pos_calc:
                            if neg[1] >= -0.01 or pos[1] <= 0.01: continue
                            transfert = min(abs(neg[1]), pos[1])
                            transferts.append({'de': neg[0], 'a': pos[0], 'montant': transfert})
                            neg[1] += transfert
                            pos[1] -= transfert

                    # 2. AFFICHAGE : CE QUE CHAQUE PERSONNE DOIT
                    st.subheader("📤 Qui doit donner ?")
                    for t in transferts:
                        with st.container(border=True):
                            st.markdown(f"💸 **{t['de']}** → **{t['montant']:.2f}€** pour **{t['a']}**")

                    # 3. AFFICHAGE : CE QUE CHAQUE PERSONNE REÇOIT
                    st.subheader("📥 Qui doit recevoir ?")
                    for t in transferts:
                        with st.container(border=True):
                            st.markdown(f"💰 **{t['a']}** ← **{t['montant']:.2f}€** de **{t['de']}**")

                    if not df_groupe.empty and transferts:
                        st.divider()
                        
                        # 1. On génère le PDF
                        pdf_output = generer_pdf_tricount(
                            groupe_actuel, 
                            df_groupe, 
                            transferts, 
                            df_groupe['Montant'].sum()
                        )
                        
                        # 2. CRUCIAL : On convertit le bytearray en bytes
                        pdf_bytes = bytes(pdf_output)
                        
                        # 3. On passe pdf_bytes au bouton
                        st.download_button(
                            label="📄 Télécharger le résumé PDF",
                            data=pdf_bytes,
                            file_name=f"Tricount_{groupe_actuel}.pdf",
            ...
            with col_histo:
                ...
                if not df_tri.empty:
                    # Filtrage par groupe actuel
                    df_groupe_h = df_tri[df_tri['Groupe'] == groupe_actuel]
                    
                    if not df_groupe_h.empty:
                        # --- CALCUL DU TOTAL ---
                        # On additionne la colonne Montant pour ce groupe
                        total_groupe = df_groupe_h['Montant'].sum()
                        
                        # Affichage du total dans un petit bandeau stylisé
                        st.metric(label=f"Dépenses totales : {groupe_actuel}", value=f"{total_groupe:.2f} €")
                        ...
                        st.info("Aucune transaction dans ce groupe.")
                else:

   ...
   # Encore plus petit et discret
    ...
    with st.sidebar:
        ...
        for c in tous_les_comptes:
            ...
            # On récupère la valeur actuelle stockée
            current_val = st.session_state.config_groupes[c].get("Couleur", "#1f77b4")
            
            # MODIFICATION : On lie le picker directement au dictionnaire du session_state
            # Cela permet aux cards de se mettre à jour même sans cliquer sur "Sauvegarder"
    ...
    index_actuel = ["Analyses", "Prévisionnel", "Gestion", "Import", "Groupes"].index(selected)
                ...
                if st.button("💾 Sauvegarder les modifications", type="primary", use_container_width=True):
                    if apprendre:
                        for idx_save, row_save in df_f.iterrows():
                            if st.session_state.df.at[idx_save, 'Categorie'] != row_save['Categorie']:
                                sauvegarder_apprentissage(row_save['Nom'], row_save['Categorie'])
                    
                    st.session_state.df.update(df_f)
                    st.session_state.df = charger_donnees(st.session_state["username"]) # Recharge direct depuis GSheets

...
# Assure-toi que ton engine est bien défini avec tes secrets Neon
engine = create_engine(st.secrets["connections"]["postgresql"]["url"])
...
def mettre_a_jour_soldes_neon(df_transactions, soldes_initiaux):
    nouveaux_soldes = {str(k).strip().upper(): float(v) for k, v in soldes_initiaux.items()}
    config = st.session_state.get('config_groupes', {})
    
    # On identifie tous les virements RÉELS présents dans les données
    # pour ne pas les compter deux fois
    virements_reels = df_transactions[df_transactions['categorie'].str.contains("🔄", na=False)]
    ...
    for _, ligne in df_transactions.iterrows():
        ...
        # 1. On applique la ligne réelle
        if compte_source in nouveaux_soldes:
            nouveaux_soldes[compte_source] += montant
        
        # 2. On gère la contrepartie UNIQUEMENT si elle n'existe pas déjà en ligne réelle
        if "🔄" in cat:
                ...
                for nom_c, cfg in config.items():
                    ...
                    if cfg.get("Groupe") == groupe_source and nom_c_upper != compte_source:
                        ...
                        if mots and mots[0] in cat:
                            ...
                            # On ne simule le virement que si on ne trouve pas 
                            # de ligne opposée dans le DataFrame
                            deja_present = not virements_reels[
                                (virements_reels['compte'].str.upper() == nom_c_upper) & 
                                (abs(virements_reels['montant']) == abs(montant))
                            ].empty
                            
                            if not deja_present:
                                if montant < 0:
                                    nouveaux_soldes[nom_c_upper] = nouveaux_soldes.get(nom_c_upper, 0) + abs(montant)
                                else:
                                    nouveaux_soldes[nom_c_upper] = nouveaux_soldes.get(nom_c_upper, 0) - abs(montant)
                            break
   ...
   # 1. On sécurise le DataFrame
    ...
    if selected == "Analyses":
            ...
            if "filtre_profil_index" not in st.session_state:
                st.session_state.filtre_profil_index = 0  # Par défaut : "Tout le monde"
            ...
            def afficher_dashboard():
                # 1. LES SÉLECTEURS (Ligne 1)
                cols_filtres = st.columns([1, 1, 1, 1])
                
                # On prépare la liste des options
                options_profil = ["Tout le monde"] + st.session_state.groupes_liste

                with cols_filtres[0]:
                    # On récupère l'index stocké pour que le selectbox revienne dessus
                    choix_actuel = st.selectbox(
                        "🎯 Profil", 
                        options_profil, 
                        index=st.session_state.filtre_profil_index,
                        key="choix_g_widget" # Clé différente pour ne pas interférer avec le stockage manuel
                    )
                            
                # --- LOGIQUE DE FILTRAGE PAR PROFIL (CORRIGÉE) ---
                if choix_actuel != "Tout le monde":
                    ...
                    soldes_finaux = mettre_a_jour_soldes_neon(df_h, soldes_depart)

                # On enregistre l'index
                st.session_state.filtre_profil_index = options_profil.index(choix_actuel)

                # --- HARMONISATION DES CATÉGORIES (DÉPLACEMENT ICI) ---
                if not df_h.empty:
                    ...
                    if col_trouvee:
                        ...
                        categories_detectees = df_h['categorie'].unique().tolist()
                    else:
                        categories_detectees = []
                        st.warning("⚠️ La colonne 'categorie' est absente.")
                else:
                    categories_detectees = []

                virements_techniques = [c for c in categories_detectees if any(x in str(c).upper() for x in ["🔄", "VERS ", "INTERNE"])]
                categories_dispo = sorted([c for c in categories_detectees if c not in virements_techniques])
                # --- CALCUL DES SOLDES RÉELS (Basés sur df_h déjà filtré par profil) ---
                soldes_depart = {str(c).strip().upper(): st.session_state.config_groupes[c].get("Solde", 0.0) for c in cps if c in st.session_state.config_groupes}
                
                # Mise à jour des années disponibles selon le profil
                annees_profil = sorted([int(a) for a in df_h['année'].dropna().unique()], reverse=True) if not df_h.empty else liste_annees

                with cols_filtres[1]:
                    annee_choisie = st.selectbox("📅 année", annees_profil)

                # Calcul des soldes finaux
                soldes_finaux = mettre_a_jour_soldes_neon(df_h, soldes_depart)
                solde_global = sum(soldes_finaux.values())

                # On prépare df_dash filtré par année
                df_dash = df_h[df_h["année"] == annee_choisie].copy()

                # --- FILTRAGE PAR mois ---
                liste_m = sorted(df_dash['mois'].unique(), key=lambda x: nomS_mois.index(x) if x in nomS_mois else 0) if not df_dash.empty else []
                with cols_filtres[2]:
                    mois_choisi = st.selectbox("📆 mois", liste_m)

                # Données réelles pour les graphiques (filtrées Profil + Année + Mois + Sans virements)
                df_reel = df_dash[
                    (df_dash["mois"] == mois_choisi) & 
                    (~df_dash["categorie"].isin(virements_techniques))
                ].copy()

                # --- 1. PRÉPARATION DES DONNÉES ANNUELLES ---
                annee_max = annee_choisie
                df_dash['date'] = pd.to_datetime(df_dash['date'], dayfirst=True, errors='coerce')

                # --- 2. CALCUL DES FLUX ANNUELS (REVENUS vs DÉPENSES) ---
                # On utilise df_dash (déjà filtré par PROFIL et ANNÉE)
                # On exclut strictement les virements pour ne pas fausser les Revenus/Dépenses
                ...
                # --- 3. CONSTRUCTION DU TABLEAU DE PATRIMOINE ---
                df_tab = pd.DataFrame({'mois': nomS_mois})

                for c in cps:
                    # On récupère le solde initial défini dans la config
                    s_init_compte = st.session_state.config_groupes.get(c, {}).get("Solde", 0.0)
                    
                    # IMPORTANT : Pour le solde des comptes, on garde TOUT (virements inclus !)
                    # Si le virement de 1000€ est bien présent en négatif sur le CCP, le cumul sera juste.
                    mouvements = df_dash[df_dash["compte"] == c].groupby("mois")["montant"].sum().reset_index()
                    
                    # Alignement sur 12 mois
                    df_c = pd.merge(pd.DataFrame({'mois': nomS_mois}), mouvements, on='mois', how='left').fillna(0)
                    
                    # Calcul cumulatif : Solde Initial + somme des mouvements mois après mois
                    df_tab[c] = s_init_compte + df_c["montant"].cumsum()

                # --- 4. FUSION DES FLUX ET SOLDES ---
                # On fusionne df_tab (soldes des comptes) avec df_ann (flux revenus/dépenses)
                df_tab = pd.merge(df_tab, df_ann[['mois', 'Revenus', 'Dépenses']], on='mois', how='left').fillna(0)
                
                # Calcul de l'épargne (basé sur flux réels)
                df_tab['Épargne'] = df_tab['Revenus'] - df_tab['Dépenses']
                
                # Calcul du Patrimoine (somme de tous les comptes du profil à chaque mois)
        ...
        if "p_choix_g" not in st.session_state: st.session_state.p_choix_g = "Tout le monde"
        if "p_choix_a" not in st.session_state: st.session_state.p_choix_a = int(time.localtime().tm_year)
        if "p_choix_m" not in st.session_state: st.session_state.p_choix_m = nomS_mois[time.localtime().tm_mon - 1]
        ...
        def afficher_zone_previsionnelle():
            ...
            # FILTRE PROFIL
            options_g = ["Tout le monde"] + st.session_state.groupes_liste
            idx_g = options_g.index(st.session_state.p_choix_g) if st.session_state.p_choix_g in options_g else 0
            
            with cols_f[0]:
                choix_g = st.selectbox("🎯 Profil", options_g, index=idx_g, key="prev_g_widget")
                st.session_state.p_choix_g = choix_g # Sauvegarde pour le retour sur page
            
            # On cherche "groupe" ou "Groupe" sans se soucier de la casse
            # --- RÉCUPÉRATION DES COMPTES DU PROFIL ---
            if choix_g == "Tout le monde":
            ...
            with cols_f[1]:
                annee_p = st.selectbox("📅 année", liste_a, index=idx_a, key="prev_a_widget")
                st.session_state.p_choix_a = annee_p

            # FILTRE MOIS
            liste_mois_select = ["Tous les mois"] + nomS_mois
            idx_m = liste_mois_select.index(st.session_state.p_choix_m) if st.session_state.p_choix_m in liste_mois_select else time.localtime().tm_mon
            
            with cols_f[2]:
                mois_p = st.selectbox("📆 mois", liste_mois_select, index=idx_m, key="prev_m_widget")
            ...
            with col2:
                ...
                # Debug interne pour voir ce qui bloque (visible uniquement si vide)
                if df_source.empty:
                    st.warning("Le DataFrame source est vide.")
                
                # --- 2. FILTRAGE ---
                if mois_p == "Tous les mois":
                    # On ne filtre QUE par utilisateur (déjà fait au chargement normalement)
                    df_mois_prev = df_source
                else:
                    # Filtrage par mois (nettoyage des espaces et casse)
                    df_mois_prev = df_source[
                ...
                if not df_mois_prev.empty:
                    ...
                    # Si c'est vide ici, on affiche pourquoi
                    st.info(f"Aucune donnée trouvée pour le mois : {mois_p}")
                    with st.expander("Voir les données brutes"):
                        st.write(df_source) # Ceci te montrera si 'theo' a bien des données et quel est leur mois
                ...
                if not df_mois_prev.empty:
                    ...
                    def toggle_all():
                        ...
                        if st.session_state.master_check:
                ...
                def zone_interactive_tableau():
                    ...
                    with col_sidebar:
                        ...
                        # --- LOGIQUE DE TRI ---
                            ...
                            def toggle_all():
                                    val = st.session_state.master_control_v3

...
def afficher_ligne_compacte(row, couleur_montant, prefixe=""):
    ...
    # On extrait l'icône (le premier caractère)
    raw_ico = cat[:1] if cat else "💰"
    
    # --- LE FIX ICI ---
    # On ajoute \uFE0F pour forcer le rendu emoji coloré
    ico = raw_ico + "\uFE0F"
    
    if any(x in cat for x in ["Virement :", "Transfert Interne"]) and "🤝" not in cat:
        ico = "🔄\uFE0F"
    
    # 3. Sécurité pour le texte
...
def charger_categories_perso():
    """Renvoie la liste TOTALE (Défaut + Perso) sans aucun filtre."""
    defaut = [
    ...
    #On vérifie si l'utilisateur est bien là
    ...
    try:
        ...
        perso = df_cat[df_cat["User"] == user]["Nom"].tolist()
        return sorted(list(set(defaut + perso)))
    except:
        # Si ça échoue ici, c'est que conn.read ne marche pas
        return sorted(defaut)

def charger_categories_masquees(username):
    """Lit la feuille des catégories que l'utilisateur veut cacher."""
    try:
        df_mask = conn.read(worksheet="Categories_Masquees", ttl=0)
        return df_mask[df_mask["User"] == username]["Nom"].tolist()
    except:
        return []

def charger_categories_visibles(username):
    """Renvoie la liste filtrée (Totale - Masquées)."""
    totale = charger_categories_perso()
    masquees = charger_categories_masquees(username)
    return [c for c in totale if c not in masquees]

def sauvegarder_preference_masquage(username, liste_a_masquer):
    """Enregistre les choix de l'utilisateur."""
    try:
        df_mask_global = conn.read(worksheet="Categories_Masquees", ttl=0)
        # On nettoie l'existant pour cet utilisateur
        if not df_mask_global.empty:
            df_mask_global = df_mask_global[df_mask_global["User"] != username]
        
        # On prépare les nouvelles lignes
        new_prefs = pd.DataFrame({
            "Nom": liste_a_masquer, 
            "User": [username] * len(liste_a_masquer)
        })
        df_final = pd.concat([df_mask_global, new_prefs], ignore_index=True)
        conn.update(worksheet="Categories_Masquees", data=df_final)
        return True
    except Exception as e:
        st.error(f"Erreur lors de la sauvegarde : {e}")
        return False
...
def sauvegarder_nouvelle_categorie(nouvelle_cat, username):
    try:
        ...
        # On ne se contente pas de sauvegarder, on rafraîchit la liste en mémoire
        # On appelle ta fonction de chargement (sans cache désormais)
        st.session_state.LISTE_CATEGORIES_COMPLETE = charger_categories_visibles(username)
...
if "username" in st.session_state:
    LISTE_CATEGORIES_COMPLETE = charger_categories_visibles(st.session_state.username)
else:
...
def sauvegarder_apprentissage_batch(liste_transactions, username):
    """
    Mémorise en une seule fois toutes les catégories pour un utilisateur.
    liste_transactions : liste de tuples ou dict [(nom, cat), (nom, cat)...]
    """
    try:
        # 1. Charger la mémoire existante de l'utilisateur
        memoire = charger_memoire(username)
        
        # 2. Mettre à jour le dictionnaire local en boucle (très rapide, c'est de la mémoire vive)
        for nom_ope, categorie in liste_transactions:
            nom_clean = simplifier_nom_definitif(nom_ope)
            memoire[nom_clean] = categorie
        
        # 3. Convertir le dictionnaire mis à jour en DataFrame
        df_user_memo = pd.DataFrame(list(memoire.items()), columns=['Nom', 'Categorie'])
        df_user_memo["User"] = username
        
        # 4. Lire TOUTE la mémoire globale (1 seul appel API)
        ...
        # 6. Mise à jour GSheets UNIQUE et vidage du cache
        conn.update(worksheet="Memoire", data=df_final)
        st.cache_data.clear()
        
    except Exception as e:
        st.error(f"Erreur lors de l'apprentissage groupé : {e}")
...
def sauvegarder_donnees(nouveau_df, username=None):
    try:
        ...
        # 2. Préparation du nouveau bloc
        nouveau_df["User"] = user_actif
        
        # 3. Filtrage des autres utilisateurs
        if not df_global.empty and "User" in df_global.columns:
        ...
        # --- MODIFICATION ICI : Priorité au nouveau_df ---
        df_final = pd.concat([nouveau_df, df_autres], ignore_index=True)
        
        # 4. Conversion Date
        df_final['Date'] = pd.to_datetime(df_final['Date'], dayfirst=True, errors='coerce')
        df_final = df_final.dropna(subset=['Date'])
        
        # --- MODIFICATION ICI : On garde 'first' (donc le nouveau_df car il est en haut) ---
        df_final = df_final.drop_duplicates(subset=['Date', 'Nom', 'Montant', 'User'], keep='first')
...
# Si l'utilisateur EST connecté
   ...
   # Encore plus petit et discret
                        ...
                        with st.container(height=430):
                           ...
                           # 1. CALCUL DE L'ÉPARGNE RÉELLE CUMULÉE (Flux uniquement, sans le solde initial)
                            ...
                            try:
                                ...
                                mes_projets_df['Date'] = pd.to_datetime(mes_projets_df['Date'], format='%d/%m/%Y', errors='coerce')
                                        ...
                                        with col_stat:
                                            with st.popover("✏️"):
                                                ...
                                                if st.button("Enregistrer les modifs", key=f"s_{index}", width='stretch', type="primary"):
                                                    ...
                                                    df_f.loc[m, 'Date'] = n_date.strftime('%d-%m-%Y')
                    ...
                    def valider_et_nettoyer():
                        ...
                        if nom:
                            ...
                            if sauvegarder_nouvelle_categorie(val_finale, st.session_state.username):
                                ...
                                st.session_state.LISTE_CATEGORIES_COMPLETE = charger_categories_visibles(st.session_state.username)
    ...
    # Popover compact avec onglets
                ...
                # --- BLOC MASQUAGE EN POPOVER ---
                with st.popover("⚙️ Gérer l'affichage des Catégories", width='stretch'):
                    st.markdown("**Catégories à afficher**")
                    st.caption("Décochez les catégories inutiles. Les changements s'appliqueront après validation.")
                    
                    # 1. On charge les données
                    toutes_les_cats = charger_categories_perso() 
                    masquees_actuellement = charger_categories_masquees(st.session_state.username)
                    
                    # 2. Utilisation d'un FORMULAIRE pour éviter le rerun à chaque clic
                    with st.form("form_masquage", border=False):
                        nouvelle_liste_masquee = []
                        
                        container_filtre = st.container(height=300)
                        with container_filtre:
                            for cat in toutes_les_cats:
                                # La checkbox ne déclenchera RIEN tant qu'on ne valide pas le formulaire
                                est_visible = st.checkbox(
                                    cat, 
                                    value=(cat not in masquees_actuellement), 
                                    key=f"pop_{cat}"
                                )
                        ...
                        # 3. Le bouton de validation du formulaire (st.form_submit_button)
                        submit = st.form_submit_button("Enregistrer", width='stretch', type="primary")
                        
                        if submit:
                            if sauvegarder_preference_masquage(st.session_state.username, nouvelle_liste_masquee):
                                # Mise à jour de la liste globale
                                st.session_state.LISTE_CATEGORIES_COMPLETE = charger_categories_visibles(st.session_state.username)
                                st.toast("Affichage mis à jour ! ✨")
                                # Ici on fait un rerun unique pour appliquer les changements à toute l'interface
                                st.rerun()
            ...
            with col_add:
                ...
                with st.form("form_reel_multi"):
                    ...
                    for idx in st.session_state.indices_reel:
                        with st.container():
                            ...
                            ops_reelles.append({
                            "Date": f_date.strftime('%d/%m/%Y'), # Sauvegarde en texte propre pour éviter l'erreur de format
                            "Nom": f_nom, 
                            "Montant": f_mnt,
                            "Categorie": f_cat, 
                            "Compte": f_compte,
                ...
                # --- LOGIQUE DE SAUVEGARDE (VERSION SÉCURISÉE) ---
                if submit_reel:
                    valides = [o for o in ops_reelles if o["Nom"].strip() != "" and o["Montant"] != 0]
                    
                    if valides:
                        # 1. Création du DataFrame des nouvelles entrées
                        df_new_ops = pd.DataFrame(valides)
                        
                        # 2. Préparation du DataFrame global (Existant + Nouveau)
                        # On récupère l'existant depuis le state
                        ...
                        # --- 3. NETTOYAGE ET CONVERSION (Anti-Erreur .dt) ---
                        try:
                            # On force la conversion en datetime (le format 'mixed' gère les différents formats texte/objet)
                            df_total['Date'] = pd.to_datetime(df_total['Date'], dayfirst=True, format='mixed', errors='coerce')
                            
                            # On supprime les lignes NaT (dates invalides) qui font planter le .dt
                            df_total = df_total.dropna(subset=['Date'])
                            
                            # Maintenant l'accesseur .dt est garanti de fonctionner
                            # On repasse en texte JJ/MM/AAAA pour le stockage final
                            df_total['Date'] = df_total['Date'].dt.strftime('%d/%m/%Y')
                            
                            # 4. Sauvegarde via ta fonction globale
                        ...
                        st.warning("⚠️ Aucune opération valide à ajouter.")
                    ...
                    with col2:
                        if st.button("Oui, Supprimer", type="primary", width='stretch'):
                            ...
                            try:
                                ...
                                df_pour_gsheet['Date'] = df_pour_gsheet['Date'].dt.strftime('%d/%m/%Y')
                ...
                with st.container(border=True):
                    ...
                    with c2:
                        # On convertit tout en texte et on retire les valeurs vides (NaN) avant le tri
            ...
            with col_main:
                ...
                if 'LISTE_CATEGORIES_COMPLETE' not in st.session_state:
                    ...
                    st.session_state.LISTE_CATEGORIES_COMPLETE = charger_categories_visibles(user)
                    ...
                    for idx, row in df_f.iterrows():
                        ...
                        with c_cat:
                            ...
                            nouvelle_cat = st.selectbox(
                                ...
                                on_change=update_df_from_ui,         # Appel de la fonction
                                args=(idx, f"cat_{idx}", 'Categorie') # Passage des paramètres
                            )
                ...
                with c_save2:
                    if st.button("💾 Sauvegarder les modifications", type="primary", width='stretch'):
                        ...
                        # On récupère le DF qui a été mis à jour en direct par les selectbox
                        df_a_sauver = st.session_state.df.copy()

                        # --- DANS TON BOUTON SAUVEGARDER ---
                        if apprendre:
                            nouvelles_regles = []
                            user_actuel = st.session_state.get("username")
                            ...
                            for idx_f in df_f.index:
                                key_cat = f"cat_{idx_f}"
                                if key_cat in st.session_state:
                                    cat_choisie = st.session_state[key_cat]
                                    # On compare avec la valeur qu'il y avait AU CHARGEMENT de la page
                                    cat_initiale = temoin.get(idx_f)

                                    if cat_initiale and cat_choisie != cat_initiale:
                                        nom_op = df_f.at[idx_f, 'Nom']
                                        nouvelles_regles.append((nom_op, cat_choisie))
                                        
                                        # Mise à jour en cascade dans le DF principal
                            ...
                            if nouvelles_regles:
                                ...
                                sauvegarder_apprentissage_batch(nouvelles_regles, user_actuel)
                                # TRÈS IMPORTANT : On vide le témoin après avoir appris 
                                # pour qu'il se régénère au prochain affichage
                                del st.session_state.df_temoin
                        try:
                            # Nettoyage des dates avant envoi
                            dates_temp = pd.to_datetime(df_a_sauver['Date'], dayfirst=True, errors='coerce')
                            # On remplit les erreurs par la date du jour pour éviter le crash
                            dates_temp = dates_temp.fillna(pd.Timestamp.now())
                            df_a_sauver['Date'] = dates_temp.dt.strftime('%d/%m/%Y')
                            ...
                            with st.spinner("Envoi vers Google Sheets..."):
                                ...
                                time.sleep(1)
                                st.rerun() # On relance proprement
                        except Exception as e:
                            st.error(f"Erreur lors de la sauvegarde : {e}")

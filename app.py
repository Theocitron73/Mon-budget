    ...
    if st.session_state.last_page != selected:
        ...
        # --- TAB IMPORT (VERSION CORRIGÉE ET SÉCURISÉE) ---
            ...
            with col_upload:
                ...
                with st.container(border=True): # border=True crée un cadre pour bien séparer la zone
                    st.markdown("🛠️ Configurer les mots-clés pour la catégorisation automatique")
                    ...
                    with col1:
                        cat_to_edit = st.selectbox("Catégorie à modifier", all_cats)
                    
                    with col2:
                        mots_actuels = dict_auto.get(cat_to_edit, [])
                        # Multiselect pour voir et supprimer des mots existants
                        nouveaux_mots = st.multiselect(
                            f"Mots-clés pour {cat_to_edit}",
                            options=mots_actuels,
                            default=mots_actuels,
                            key=f"multi_{cat_to_edit}"
                        )

                    # 3. Ajouter un nouveau mot
                    # Correction de l'argument vertical_alignment
                    c1, c2 = st.columns([3, 1], vertical_alignment="bottom")

                    nouveau_m = c1.text_input("Nouveau mot-clé", key="input_new_mot", placeholder="Ex: NETFLIX").upper()

                    if c2.button("➕ Ajouter / Sauver", use_container_width=True):
                        if nouveau_m and nouveau_m not in nouveaux_mots:
                            nouveaux_mots.append(nouveau_m)
                        
                        if update_mots_cles_neon(cat_to_edit, nouveaux_mots):
                            st.success(f"Règles mises à jour pour {cat_to_edit} !")
                            st.rerun()

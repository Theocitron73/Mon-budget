...
def afficher_ligne_compacte(row, couleur_montant, prefixe=""):
    ...
    # 1. On essaie de prendre le premier caractère comme icône
    ico = cat[:1] if cat else "💰"
    
    # 2. Correction : On ne force le 🔄 QUE SI l'émoji actuel n'est pas déjà 🤝 ou 💰
    # et que c'est un transfert technique.
...
# Si l'utilisateur EST connecté
   ...
   # Encore plus petit et discret
    ...
    with st.sidebar:
        ...
        # --- 4. APPLICATION VISUELLE IMMÉDIATE ---
        st.markdown(f"""
            <style>
            .stApp {{
    ...
    if selected == "Analyses":
        ...
        # A. Préparation des données combinées (Réel + Prévu)
        # On prend tout le réel jusqu'à aujourd'hui + le prévisionnel
        ...
        # B. Calcul des soldes par compte avec la logique "Virements Internes"
        # Initialisation des soldes avec les données réelles déjà calculées
        # (ou les soldes de base si tu pars de zéro)
        # --- 3. CALCULS ---
        # Initialisation
        soldes_finaux_comptes = {}
        mois_idx_fin = NOMS_MOIS.index(mois_p) if mois_p != "Tous les mois" else 11
        
        # Pré-filtrage des données pour gagner en performance
        df_reel_filtre = st.session_state.df[st.session_state.df["Compte"].isin(cps)].copy()
        df_prev_filtre = st.session_state.df_prev[st.session_state.df_prev["Compte"].isin(cps)].copy()

        for c in cps:
            nom_c_upper = str(c).strip().upper()
            # On récupère le solde initial défini dans ta configuration
            solde_initial = st.session_state.config_groupes.get(c, {}).get("Solde", 0.0)
            
            # A. CALCUL DU RÉEL (Historique)
            # --- CORRECTION ICI : Conversion forcée avant le filtre ---
            df_reel_filtre["Date"] = pd.to_datetime(df_reel_filtre["Date"], errors='coerce')
            
            cond_date_r = (df_reel_filtre["Année"] < annee_p) | \
                        ((df_reel_filtre["Année"] == annee_p) & (df_reel_filtre["Date"].dt.month <= mois_idx_fin + 1))
            
            m_reel_direct = df_reel_filtre[(df_reel_filtre["Compte"] == c) & cond_date_r]["Montant"].sum()

            # B. CALCUL DU PRÉVISIONNEL (avec logique Miroir comme dans ton code réel)
            cond_date_p = (df_prev_filtre["Année"] < annee_p) | \
                        ((df_prev_filtre["Année"] == annee_p) & (df_prev_filtre["Mois"].apply(lambda x: NOMS_MOIS.index(x) <= mois_idx_fin)))
            
            df_p_periode = df_prev_filtre[cond_date_p]
            ...
            for _, ligne in df_p_periode.iterrows():
                ...
                # 1. Impact direct : la prévision est sur ce compte
                if compte_source == nom_c_upper:
                    m_prev_total += montant
                
                # 2. Impact indirect : c'est un virement VERS ce compte
                elif "🔄" in cat or "VERS" in cat:
                    # On reprend ta logique : on cherche le premier mot du compte dans la catégorie
                    mots_compte = [m for m in nom_c_upper.split() if len(m) > 2]
                    if mots_compte and mots_compte[0] in cat:
                        # Si c'est une sortie d'argent du compte source, c'est une entrée ici
                        m_prev_total += abs(montant) if montant < 0 else -abs(montant)

            soldes_finaux_comptes[c] = solde_initial + m_reel_direct + m_prev_total

        # Calcul du patrimoine global (neutre sur les virements)
        total_base_global = sum([st.session_state.config_groupes[c].get("Solde", 0.0) for c in cps])
        
        # Identification des virements pour le global (pour ne pas les compter deux fois)
        categories_detectees = st.session_state.df['Categorie'].unique().tolist() if not st.session_state.df.empty else []
        virements_techniques = [cat for cat in categories_detectees if "🔄" in str(cat) or "VERS" in str(cat).upper()]
        
        df_combi_sans_vir = pd.concat([
            df_reel_filtre[~df_reel_filtre["Categorie"].isin(virements_techniques)],
            df_prev_filtre[~df_prev_filtre["Categorie"].isin(virements_techniques)]
        ])
        
        flux_global = df_combi_sans_vir[
            (df_combi_sans_vir["Année"] < annee_p) | 
            ((df_combi_sans_vir["Année"] == annee_p) & 
            (df_combi_sans_vir["Mois"].apply(lambda x: NOMS_MOIS.index(x) if x in NOMS_MOIS else 0) <= mois_idx_fin))
        ]["Montant"].sum()
        
        patrimoine_global_projete = total_base_global + flux_global

        # Recréation de df_combi pour les graphiques et listes (colonnes 2 et 3)
        df_combi = pd.concat([
            st.session_state.df[(st.session_state.df["Compte"].isin(cps)) & (st.session_state.df["Année"] == int(annee_p))],
            st.session_state.df_prev[(st.session_state.df_prev["Compte"].isin(cps)) & (st.session_state.df_prev["Année"] == int(annee_p))]
        ], ignore_index=True)
        
        # Point de départ pour le graphique de cumul
        ...
        # Flux total net (Revenus - Dépenses hors virements)
        flux_global = df_combi_sans_vir[
            (df_combi_sans_vir["Année"] < annee_p) | 
            ((df_combi_sans_vir["Année"] == annee_p) & 
            (df_combi_sans_vir["Mois"].apply(lambda x: NOMS_MOIS.index(x) if x in NOMS_MOIS else 0) <= mois_idx_fin))
        ...
        patrimoine_global_projete = total_base_global + flux_global
        # Re-création de df_combi pour la suite du code (Affichage et Graphiques)
        df_combi = pd.concat([
            st.session_state.df[(st.session_state.df["Compte"].isin(cps)) & (st.session_state.df["Année"] == int(annee_p))],
            st.session_state.df_prev[(st.session_state.df_prev["Compte"].isin(cps)) & (st.session_state.df_prev["Année"] == int(annee_p))]
        ], ignore_index=True)
        
        # Sécurité pour s'assurer que les dates sont bien au format datetime
        df_combi['Date'] = pd.to_datetime(df_combi['Date'])
                ...
                if not df_mois_prev.empty:
                    for idx, r in df_mois_prev.sort_values("Date").iterrows():
                        ...
                        if c3.button("🗑️", key=f"del_{idx}"):
                            ...
                            sauvegarder_previsions(st.session_state.df_prev)
                            st.rerun()
        ...
        with col3: 
            # Ton code actuel pour df_tab_p...
            df_tab_p = pd.DataFrame({'Mois': NOMS_MOIS})

        with col3: 
            # (Le reste du code pour les graphiques et le tableau récapitulatif reste identique 
            # car il utilise df_tab_p calculé à partir de st.session_state.df_prev déjà mis à jour)
            df_tab_p = pd.DataFrame({'Mois': NOMS_MOIS})
            stats = df_combi.groupby('Mois')['Montant'].agg(Rev=lambda x: x[x>0].sum(), Dep=lambda x: abs(x[x<0].sum())).reset_index()
            ...
            st.markdown(f"""<div style="display:flex; font-size:9px; color:gray; font-weight:bold; padding:5px 10px;">
                        <span style="width:60px;">MOIS</span><span style="flex:1;text-align:right;">REVENUS</span>
                        <span style="flex:1;text-align:right;">DÉPENSES</span><span style="flex:1;text-align:right;">ÉPARGNE</span>
                        <span style="flex:1.2;text-align:right;">SOLDE</span></div>""", unsafe_allow_html=True)
            
            with st.container(height=280):
                for _, row in df_tab_p.iterrows():
                    bg = "rgba(243, 156, 18, 0.12)" if row['Mois'] == mois_p else "transparent"
                    color_ep = col_epargne if row['Epargne'] >= 0 else "#ff4b4b"
                    
                    st.markdown(f"""
                    <div style="display:flex; padding:6px 10px; background:{bg}; border-radius:4px; margin-bottom:2px; font-size:11px; border-bottom:1px solid rgba(128,128,128,0.05);">
                        <span style="width:60px; font-weight:bold; color:#7f8c8d;">{row['Mois']}</span>
                        <span style="flex:1; text-align:right; color:{col_rev};">{row['Rev']:,.0f}</span>
                        <span style="flex:1; text-align:right; color:{col_perf_dep};">{row['Dep']:,.0f}</span>
                        <span style="flex:1; text-align:right; color:{color_ep}; font-weight:bold;">{row['Epargne']:,.0f}</span>
                        <span style="flex:1.2; text-align:right; color:{col_patri}; font-weight:bold;">{row['Solde']:,.0f}€</span>
            ...
            with col_main:
                ...
                with st.container(height=650, border=True):
                    for idx, row in df_f.iterrows():
                        ...
                        with c_cat:
                            df_f.at[idx, 'Categorie'] = st.selectbox("C", options=LISTE_CATEGORIES_COMPLETE, 
                                                                    index=LISTE_CATEGORIES_COMPLETE.index(row['Categorie']) if row['Categorie'] in LISTE_CATEGORIES_COMPLETE else 0, 
                ...
                if st.button("💾 Sauvegarder les modifications", type="primary", use_container_width=True):
                    # 1. Appliquer les modifications du formulaire (df_f) au DataFrame principal
                    st.session_state.df.update(df_f)
                    
                    # 2. Apprentissage (seulement si coché)
                    if apprendre:
                        for idx_save, row_save in df_f.iterrows():
                            nom = str(row_save.get('Nom', '')).strip()
                            cat = str(row_save.get('Categorie', '')).strip()
                            
                            # On ne sauvegarde que si les données sont valides et non vides
                            if nom and cat and nom.lower() != 'nan' and cat.lower() != 'nan':
                                sauvegarder_apprentissage(nom, cat)
                    
                    # 3. SAUVEGARDE RÉELLE (Envoi vers Google Sheets)
                    # Remplacer 'sauvegarder_donnees' par le nom exact de votre fonction de sauvegarde
                    sauvegarder_donnees(st.session_state.df, st.session_state["username"])
                    
                    # 4. Nettoyage du cache et rafraîchissement
                    st.cache_data.clear()
                    st.success("✅ Modifications enregistrées et mémorisées !")
                    st.rerun()
